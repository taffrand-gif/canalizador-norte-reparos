...
| 2026-08-14 | cowork-loop | **R11/R12 (priorité sur la tâche prévue) — ré-application de la PR #300, mergée puis PERDUE : 17 classes Tailwind cassées sur 11 fichiers** | Contrôle de début de run prescrit par `context.md` (`grep -rn 'A confirmar-' client/src` doit valoir **0**) : le compteur est ressorti à **17**, sur les **mêmes 11 fichiers** que la PR #300 du 13/08. Or `gh pr view 300` déclare `state=MERGED, mergedAt=2026-08-13T01:33:15Z`. Diagnostic : le merge commit `5b187a3430` **n'est ancêtre d'aucune branche** (`git merge-base --is-ancestor 5b187a3430 github/main` → faux ; `git branch -a --contains` → vide). `main` a donc été **réécrit après le 13/08 01:33 UTC**, ce qui a silencieusement annulé la restauration. Audit de contrôle sur les **25 dernières PR mergées** : **#300 est la seule perdue** (24/25 ancêtres de `main`) → incident isolé, pas systémique. Impact production ré-ouvert : `ui/menubar`, `ui/dropdown-menu`, `ui/context-menu` perdaient `z-50` (**menus rendus sous le contenu**), `Header.tsx` perdait la **cible tactile 44 px** (accessibilité mobile), `Contactos.tsx` perdait bordure et hauteur mini du `textarea`. Origine première inchangée : `e9782a3498` (PR #215), purge du motif `<N> min` appliquée à l'intérieur des chaînes `className` (`border-2 min-h-32` → `border-A confirmar-h-32`). **Restauration verbatim, zéro invention (R4)** : `git checkout 5b187a3430 -- <fichier>` fichier par fichier. Pré-contrôle : les 11 fichiers de `github/main` sont **octet pour octet identiques** au parent du merge perdu (`a6ff2ae0f4`), donc la ré-application est exacte et sans conflit — aucune modification tierce n'a été écrasée. | R11/R12 (violation détectée = priorité sur la tâche prévue), R4 (zéro invention — restauration verbatim depuis le merge commit d'origine), R8 (témoins avant/après, 1 motif par commande), R6 (aucun force-push), R7 (zéro merge), commit atomique 1 fichier = 1 commit, R-WT (worktree dédié, checkout partagé non modifié) | 11 commits, 11 fichiers de production. Témoins R8 : `A confirmar-` **17→0** · `min-w-` **19→33** · `min-h-` **137→140** · `z-50` **28→31** · `A confirmar` total **234→217** (contrôle positif : la purge de contenu n'est pas défaite). `./node_modules/.bin/tsc --noEmit` : **0 erreur** sur les 11 fichiers. ⚠️ **Baseline tsc à corriger dans `context.md`** : le total n'est plus **215** mais **322**, mesuré à l'identique sur `github/main` intact ET sur la branche — la dérive vient des merges intermédiaires, pas de ce patch. Le contrôle de sanité reste valable, la constante doit être mise à jour. Branche `loop/2026-08-14-canalizador-norte-reparos-r11-restaure-pr300` depuis `github/main`, en **worktree** sous `~/work/Sites/_worktrees/loop-2026-08-14/cnr-r11`. | ⏳ PR ouverte |
