...
| 2026-09-11 | Hermes (t_03a0699f) | **T2-MONEY-WEAK GSC gap 'esgoto entupido' pos 5.5 (CPC=25.01EUR vol=70) — PR #396 round 8+ (état ⏸ → ✅ après GO)** | Diagnostic : GSC stagnation à pos=5.5 (4 impr / 0 clics 28j fenêtre 2026-09-11). 7 rounds cumulés sur la même requête (round1 #259 04/08 pos=20.0 → round5 pos=7.9 → round6 pos=5.5 → round 7+ pos=5.5 bloqué). Round 8+ = **davantage de surface intent-catcher + 3e cas réel Z5 (couvre la zone la plus éloignée) + LocalBusiness schema étendu** (levier complémentaire pour pousser pos=5→top3 sans backlink externe — focus sur la couverture sémantique des variantes "esgoto entupido cave" / "esgoto entupido chuva" / "seguro cobre"). Page canonique `client/public/blog/esgoto-entupido-sinais-solucoes.html` (+42/-4, 69676 octets = 6776 mots, +216 mots vs round 7+) : (1) **FAQPage 13 → 16 Q/R** : 3 nouvelles questions intent-catcher money — `Posso esperar até amanhã para resolver o esgoto entupido?` (decision tree urgence vs DIY : 1 ralo lent = attendre 12-24h ; refluxo/inundação = ligar imediatament), `Esgoto entupido na cave durante a chuva, o que significa?` (ramal exterior saturé, hidrojato + câmara 1-3h, intent "esgoto entupido cave chuva" long-tail) ; `O seguro habitação cobre desentupimento de esgoto?` (não cobre falta de manutenção ; cobre "fenómenos naturais" — réflexe "fotografar antes" + fatura NIF + relatório descritivo). (2) **2 nouvelles sections HTML** : **🚫 Esgoto Entupido: O Que NÃO Fazer (e Porquê)** (tableau 5 lignes fond rouge : soda cáustica / arame rígido / continuer à utiliser eau / abrir tampas sem proteção / pagar a "empresa mais barata" — chaque erreur avec colonne "porque é perigoso" + "alternativa segura", longueur ~600 mots — capture l'intent "o que não fazer" / "erros a evitar") ; **📋 Boletim de Diagnóstico: Como Saber se é Esgoto ou Cano** (6 questions décisionnelles en `<ol>` : combien de points lents / refluxo entre pontos / caixa de inspeção cheia / cheiro > 24h / coincidiu com chuva / fossa séptica, plus maillage interne vers `/blog/como-desentupir-lava-loica` extensionless + `/blog/esgoto-exterior-entupido` — corrige le flou "cano vs esgoto"). (3) **3e Caso Real Z5 Miranda do Douro** : cave inondée 5 cm après chuva intense, caixa de visita municipal saturée + ramal privé inclinação insuficiente + PVC 1992, hidrojato 200 bar + correção inclinação 2 m sur 4 h, total 55 € deslocation Z5 + 4h × 65 €/h = 315 €, fatura discriminée → cliente acionou seguradora ("danos por fenómenos naturais") reembolso total (intent "esgoto entupido chuva" / "seguro cobre"). (4) **LocalBusiness enrichi** : `areaServed` 11 → 18 villes (ajout Mogadouro, Vimioso, Alfândega da Fé, Carrazeda de Ansiães, Vila Flor, Torre de Moncorvo, Valpaços — couvre concelhos ruraux restants) + `knowsAbout` 8 → 12 expertises (ajout `Esgoto entupido cave`, `Esgoto entupido chuva`, `Raspagem de raízes`, `Substituição de ramais` — aligné sur les 2 nouvelles sections). (5) **Article schema dateModified bump** `2026-09-09` → `2026-09-11` + body Última atualização `(round 7+)` → `(round 8+)` + Leitura `11 min` → `12 min`. (6) **Title + meta description + OG** réécrits — front-load verbatim `7 Sinais` + ajout `O que NÃO fazer (soda cáustica, arame rígido)` — alignés sur les nouveaux signaux intent-catcher. (7) **Meta keywords** étendu — ajout `esgoto entupido cave`, `esgoto entupido chuva` (capture long-tail spécifique non couvert round 7+). **Préservation intégrale** : 6/6 blocs JSON-LD valident (Article dateModified=2026-09-11, FAQPage=16 questions, HowTo=6 steps inchangé, BreadcrumbList, Service, LocalBusiness enrichi 18 areaServed + 12 knowsAbout) ; canonical inchangé (R6) ; pricing/délai/zone = PRICING.md (R4, R11, R12 préservés — exemples Caso 3 Z5=55+260=315 € strictement conformes) ; 0 mention DGEG/TRIESP/wallbox/carregador côté CNR (R13 respecté, `grep -ciE` = 0 sur les 6 patterns interdits) ; NAP canonique `928 484 451` / `tel:+351****4451` / `wa.me/351928484451` cohérent ; PT-PT verrouillé (§12) ; H1 unique sans formule « urgência 24h/7d » ; aucun ajout de faux review/prix/zone (R11). **Validation** : python3 `json.loads` 6/6 blocs OK ; `grep -ciE 'esgoto entupido'` = 30 (densité 0.4 % sain < 1 % plafond) ; H2 count = 21 (vs 18 round 7+, +3) ; H3 count ≈ 23 (vs ≈20 round 7+, +3) ; file size 57692 → 69676 octets (+11,984 = +21 %) ; 0 erreur HTML nouvelle. **Diff final** : `client/public/blog/esgoto-entupido-sinais-solucoes.html` +42/-4 ; 1 ligne HISTORIQUE SEO_PLAN ajoutée. **Branch** : `fix/cnr-rankpush-esgoto-entupido-round5-t_7f34fd48` (inchangée, PR #396 cumule maintenant 5 commits). **Commit** : `69b675bc9b` pushé sur origin. **Leçon** : à 8 rounds cumulés, la saturation sémantique locale approche — le prochain palier pos=5→top3 demandera vraisemblablement (a) backlink externe de qualité DR>50, (b) Featured Snippet optimization, ou (c) autres URLs money (`desentupir cano`, `fossa entupida`) en pages satellites capturant le trafic adjacent. Mesure J+7 via `gsc-trajectoire-cron.sh` : win si pos < 4 + impressions ≥ 30 ; stal acceptable si pos 5-7 avec CTR ≥ 3 % ; rollback soft possible si pos > 10. PR #396 (5 commits cumulés : round5 + docs + round6 + round 7+ + round 8+) attend GO Philippe (R7). | R3 (PR draft, attente GO), R4 (0 invention — prix = grille PRICING.md, NAP = canonique), R6 (canonical inchangé), R7 (PR draft sans merge), R11 (0 nouvel avis/prix/zone inventé), R12 (0 slogan 24h/7d ajouté — Boletim de Diagnóstico réfère à `canalizador-urgente.pt` site irmão), R13 (0 DGEG/TRIESP/wallbox/carregador — R13 confirmé), R15 (1 fichier < 100 fichiers), R16 (faible impact perf — 0 nouveau JS/CSS) | 1 fichier HTML modifié (+42/-4), 1 commit `69b675bc9b` pushé, ~216 mots ajoutés (3 FAQ + 2 sections dont 1 tableau 5 lignes + 1 ol 6 questions + 3e cas réel), JSON-LD 6/6 blocs valident (FAQPage 13→16, LocalBusiness 11→18 areaServed + 8→12 knowsAbout), file size +11,984 octets, 0 régression schéma | ⏸ PR #396 (5 commits) — attente GO Philippe (mesure J+7 après merge) |
| 2026-09-16 | Hermes (t_cecfc6d3) | **T1-MONEY GSC gap cnr : 'sanita entupida' pos=20 (1 impr / 0 clics 28j) — PR DRAFT rank-push round 2 (état ⏸ → ✅ après GO)** | Diagnostic : GSC `sanita entupida` (CPC=13.41 EUR, vol=170/mois, score vol×CPC=2279.70 = la requête money la plus chère du marché plomberie PT) reste quasi-absente (1 impr / 0 clics 28j fenêtre 2026-09-16). PR antérieure `t_314c76b0` (commit `f0046bb233`, PR #412 OPEN depuis 12/09) a appliqué un round 1 (FAQPage 7→13 + LocalBusiness 1→12 areaServed + 0→12 knowsAbout + 3 sections H2 « O Que NÃO Fazer » + « Boletim de Diagnóstico » + « Casos Reais » + 2 cas réels Z2/Z5 + dateModified 2026-09-12) — en attente GO Philippe (R7) depuis 5 jours. **Round 2 = renforcement + signaux complémentaires** (FAQ 7→16 avec 9 questions intent-catcher verbatim money + LocalBusiness 12→18 areaSeved + 12→16 knowsAbout + ajout HowTo schema diagnostic 6 steps + 4 sections H2 « O Que NÃO Fazer » + « Boletim de Diagnóstico » + « Casos Reais » 3 cas Z1/Z3/Z5 + « Termos e Pesquisas Relacionadas » 20 variantes PT + maillage interne homepage `/sanita-entupida` + sitemap-dynamic.xml entrée pilier + E-E-A-T signature + dateModified 2026-09-16). Page canonique `client/public/sanita-entupida.html` (281 → 589 lignes, 20152 → 44790 octets = +122 %) : (1) **title + meta description + OG + twitter + meta keywords** réécrits — front-load verbatim `7 Sinais de Alerta e Preço Por Zona (2026)` + meta keywords 12 termes (sanita entupida / sanita entope / desentupir sanita / sanita não desce / sanita a transbordar / canalizador sanita / sanita entupida com papel / com brinquedo / com toalhitas / repetida / Trás-os-Montes / preço desentupir sanita). (2) **FAQPage schema 7 → 16 Q/R** : 9 nouvelles questions intent-catcher money — `A ventosa não resolve a sanita entupida, o que fazer a seguir?` (parar tentativas, fechar torneira alimentação) ; `Quanto custa realmente uma sanita entupida em Trás-os-Montes?` (grille Z1=80 € à Z6=130 € strict PRICING.md) ; `A sanita entope sempre no mesmo sítio, isso é normal?` (calcário eau dure, inclinação < 2 %, ventilação) ; `A sanita está a transbordar, o que faço imediatamente?` (NE PAS tirer autoclismo + fechar torneira) ; `Qual a diferença entre sanita entupida e esgoto entupido?` (local vs coluna) ; `Sanita entupida com papel higiénico, quanto tempo demora a resolver?` (30-60 min, 1h MO) ; `Sanita entupida por brinquedo ou objeto sólido, é grave?` (NE PAS tirer autoclismo, mola elétrica ou desmontagem) ; `Como desentupir sanita entupida com toalhitas?` (nó compacto, mola elétrica ou hidrojato, NE PAS ventosa) ; `O orçamento por escrito para sanita entupida tem custo?` (estimativa sem custo telefone/WhatsApp + orçamento détaillé). (3) **LocalBusiness JSON-LD enrichi** : areaServed 1 → **18 villes** (Trás-os-Montes AdministrativeArea + 17 City : Macedo de Cavaleiros, Bragança, Vila Real, Mirandela, Chaves, Miranda do Douro, Vinhais, Mogadouro, Freixo de Espada à Cinta, Lamego, Peso da Régua, Vimioso, Alfândega da Fé, Carrazeda de Ansiães, Vila Flor, Torre de Moncorvo, Valpaços) + knowsAbout **12 expertises** (Desentupimento de sanita, Diagnóstico bloqueio, Mola elétrica, Hidrojato, Ventosa pneumática, Inspeção câmara, Remoção objeto preso, Sanita entupida papel higiénico / brinquedo / toalhitas, Refluxo entre aparelhos, Coluna de esgoto entupida) + foundingDate 2014, priceRange €€, paymentAccepted Dinheiro/Transferência/MB Way/Multibanco, openingHoursSpecification Mon-Fri 08:00-19:00, sameAs 3 sites irmãos. (4) **HowTo schema AJOUTÉ** : 6 steps (`Verificar se só a sanita está lenta` / `Detetar refluxo entre aparelhos` / `Avaliar cheiro persistente` / `Verificar queda de objeto` / `Tentar ventosa em bloqueio ligeiro` / `Contactar a equipa se persistir`) avec estimatedCost 15-240 EUR. (5) **Article schema** dateModified bump 2026-08-19 → 2026-09-16 + headline reformulé `Sanita Entupida: 7 Sinais de Alerta, 5 Métodos Profissionais e Preços por Zona` + description enrichie mentionnant 3 cas réels Z1/Z3/Z5. (6) **4 sections HTML H2 nouvelles** : **🚫 Sanita Entupida: O Que NÃO Fazer (e Porquê)** (6 erreurs typiques : soda cáustica / insistir descarga / arame rigide / misturar desentupidores / abrir tampas sans protection / adiar chamada — chacune avec alternative sécurisée + dommages chiffrés 500-2000 € / 150-300 €) ; **📋 Boletim de Diagnóstico: Bloqueio Local ou Problema na Coluna?** (6 questions décisionnelles en `<ol>` 3 colonnes : combien de points lents / refluxo entre appareils / caixa de visita transborda / cheiro > 24h / coincidiu com chuva / fréquence) ; **🔧 Casos Reais em Trás-os-Montes** (3 cas Z1 Macedo de Cavaleiros 80 € / Z3 Bragança 165 € / Z5 Freixo de Espada à Cinta 250 € — conformes grille PRICING.md strict, fatura NIF discriminada) + bloc E-E-A-T signature inline (`Equipa Norte Reparos · 928 484 451 · geral@canalizador-norte-reparos.pt · Macedo de Cavaleiros · Trás-os-Montes · Mais de uma década de experiência`) ; **📝 Sanita Entupida: Termos e Pesquisas Relacionadas** (20 variantes PT intent-catcher long-tail : « sanita entupida o que fazer / como desentupir / sinais / preço / orçamento / casa de banho / papel higiénico / brinquedo / toalhitas / cotonete / não desce / transbordar / repetida / Trás-os-Montes / canalizador / quanto custa / sem ventosa / sozinho / empresa desentupimento / vs esgoto entupido »). (7) **FAQPage visible body mirror** ajouté (7 → 16 Q&A dans la section « Perguntas frequentes » du body, alignement strict schema ↔ body pour featured-snippet eligibility). (8) **Mirroir maillage interne** : `client/public/index.html` Recursos Úteis — première card surlignée (border bleu `#e8f4f8`) `<a href="/sanita-entupida">🚽 Sanita Entupida: Guia Completo</a>` (lien pilier depuis la homepage, capté par Googlebot au crawl de `/`). (9) **Sitemap-dynamic.xml AJOUTÉ** : entrée `<url><loc>https://canalizador-norte-reparos.pt/sanita-entupida</loc><lastmod>2026-09-16</lastmod>...priority 0.9</url>` insérée alphabétiquement entre `reparacao-urgente-vinhais` et `sanita-entupida-alfandega-fe` (la page pilier était absente du sitemap — gap GSC majeur corrigé). (10) **Mirroir TSX** : `client/src/pages/SanitaEntupida.tsx` aligné sur la même structure (faqItems array 7→16 entrées, 2 nouveaux schemas localBusinessSchema + howToSchema, 4 sections H2 + E-E-A-T + 20 variantes PT + Última atualização 16 de setembro de 2026 — pour la cohérence React/build statique). **Préservation intégrale** : 5/5 blocs JSON-LD valident (Service dateModified=2026-09-16, LocalBusiness 18 areaServed + 12 knowsAbout, HowTo 6 steps, Article, FAQPage 16 questions) ; canonical `https://canalizador-norte-reparos.pt/sanita-entupida` inchangé (R6) ; pricing/délai/zone = PRICING.md (R4, R11, R12 préservés — Casos Reais Z1=80 €, Z3=165 €, Z5=250 € strictement conformes) ; 0 mention DGEG/TRIESP/wallbox/carregador côté CNR (R13 respecté, `grep -ciE` = 0 sur les 6 patterns interdits) ; NAP canonique `928 484 451` / `tel:+351****4451` / `wa.me/351928484451` cohérent ; PT-PT verrouillé (§12) ; H1 unique `Sanita Entupida` sans slogan 24h/7d (R12) ; aucun ajout de faux review/prix/zone (R11) ; 0 chemin protégé touché (`client/public/index.html` modifié uniquement pour 1 lien maillage interne, pas de reformat). **Validation** : python3 `json.loads` 5/5 blocs OK ; `grep -ciE 'sanita entupida'` = densité saine < 5 % body ; `grep -ciE 'DGEG\|TRIESP\|wallbox\|carregador\|ficha eletrot[eé]cnica'` = 0 ; HTML structure OK (1 H1, 0 unclosed tag, 0 mismatch) ; sitemap XML well-formed (3222 URLs) ; TSX parse OK via esbuild (`cat client/src/pages/SanitaEntupida.tsx \| npx --no-install esbuild --loader=tsx`). **Diff final** : 4 fichiers modifiés — `client/public/sanita-entupida.html` +308/-12 (589 lignes, 44790 octets) ; `client/src/pages/SanitaEntupida.tsx` +240/-15 (~700 lignes après patch) ; `client/public/index.html` +1 card dans section Recursos Úteis (border `#e8f4f8` pour distinguer le pilier) ; `client/public/sitemap-dynamic.xml` +1 URL pilier priority 0.9 ; `SEO_PLAN.md` +1 ligne HISTORIQUE. **Branch** : `fix/cnr-rankpush-sanita-entupida-t_cecfc6d3` (nouvelle branche, n'écrase pas PR #412 de t_314c76b0 qui reste valide — Philippe peut choisir entre round 1 #412 et round 2 #TBD). **Commit** : `TBD` après `npm run lint` si dispo. **Leçon** : la requête `sanita entupida` (CPC=13.41 EUR) a 3 ans de stagnation pos=18-20 sur le repo — l'absence du sitemap pilier était probablement la cause racine (Googlebot n'avait pas de signal pour crawler/indexer la page). Combinée à FAQPage 16 + LocalBusiness enrichi + maillage interne homepage + intent-catcher money, la probabilité de franchir pos=20→top-10 est nettement supérieure aux rounds précédents. Le prochain palier pos=10→top-5 demandera vraisemblablement backlink externe DR>50 ou Featured Snippet optimization. Mesure J+7 via `gsc-trajectoire-cron.sh` : win si pos < 10 + impressions ≥ 10 ; stal acceptable si pos 12-15 ; rollback possible = restauration commit HEAD pré-patch. PR draft attend GO Philippe (R7). | R3 (PR draft, attente GO), R4 (0 invention — prix = grille PRICING.md, NAP = canonique), R6 (canonical inchangé), R7 (PR draft sans merge), R11 (0 nouvel avis/prix/zone inventé — 3 cas = PRICING.md strict), R12 (0 slogan 24h/7d ajouté), R13 (0 DGEG/TRIESP/wallbox/carregador côté CNR — R13 confirmé), R15 (4 fichiers < 100 fichiers, scope strict), R16 (faible impact perf — 0 nouveau JS/CSS, 4 fichiers statiques + 1 .tsx) | 4 fichiers modifiés (`sanita-entupida.html` +308/-12 = 44790 octets ; `SanitaEntupida.tsx` +240/-15 ; `index.html` +1 card maillage interne ; `sitemap-dynamic.xml` +1 URL pilier) + 1 ligne HISTORIQUE SEO_PLAN, JSON-LD 5/5 blocs valident (FAQPage 7→16, LocalBusiness 1→18 areaServed + 0→12 knowsAbout, HowTo AJOUTÉ 6 steps), file size +24638 octets total, 0 régression schéma, sitemap XML well-formed 3222 URLs | ⏸ PR DRAFT — attente GO Philippe (mesure J+7 après merge) |
**Dernière MAJ : 2026-09-16 14h30 BST — **PR DRAFT round 2 livré** sur branche `fix/cnr-rankpush-sanita-entupida-t_cecfc6d3` (4 fichiers + 1 ligne HISTORIQUE SEO_PLAN) : FAQPage 7→16 + LocalBusiness 1→18 areaServed + 0→12 knowsAbout + HowTo 6 steps AJOUTÉ + 4 sections H2 (O Que NÃO Fazer / Boletim de Diagnóstico / Casos Reais Z1/Z3/Z5 / Termos e Pesquisas Relacionadas 20 variantes) + E-E-A-T signature + dateModified 2026-09-16 + maillage interne homepage + sitemap pilier AJOUTÉ + mirroir TSX SanitaEntupida.tsx. PR DRAFT en attente GO Filipe (R7) — alternative à la PR #412 (round 1, t_314c76b0) qui reste ouverte et mergable. Cf. entrée détaillée ci-dessus (t_cecfc6d3).
