...
| X-R12 | « mesma pessoa » / « mesmo técnico » servis en production | HAUTE | A_FAIRE | — | — | `mesma pessoa\|mesmo t[ée]cnico` · **production CNR 2** — `public/.well-known/ai-plugin.json` · `tools/verify-busca-fuga-pilot.py`. Ce second est un **faux positif** : la locution y vit dans une regex de DÉTECTION `pronoun/solo`, il la cherche, il ne la produit pas. **Travail réel : 1 fichier.** `AGENTS.md` CNR ne porte AUCUNE prescription (0 occurrence) — contrairement à CU/EU. Contrôle positif `mesm` = 836. R12 verrouillée « nós » / « a nossa equipa » (arbitrage Filipe 29/07/2026). |
| X-MAIL | Email `privaterelay.appleid.com` publié comme contact | HAUTE | A_FAIRE | — | — | `privaterelay\.appleid\.com` · **production CNR 3** — `client/public/.well-known/ai-plugin.json` · `client/public/.well-known/security.txt` · `public/.well-known/ai-plugin.json`. Contrôle positif `appleid` = 4. |
