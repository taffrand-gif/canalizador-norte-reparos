...
-->
| X-PUB0 | Racine `public/` non servie mais déclarée servie | HAUTE | A_FAIRE | — | — | `git ls-tree -r --name-only <remote>/main -- public/` · **37 fichiers CNR** (ENR 40), **7 chemins en commun** avec `client/public/` (ENR 12), aucun servi. Prod vérifiée 02/09, 2 témoins, empreintes exactes : `/robots.txt` = `client/public/` (828 o) ≠ racine (764 o) ; `/.well-known/ai-plugin.json` = `client/public/` (1135 o) ≠ racine (1641 o). `served.json` les déclarait servis — corrigé, mais les fichiers restent : ils font gonfler tout recensement et un agent qui les lit croit patcher de la prod. **Traverse X-R12 et X-MAIL** : à traiter comme famille propre, pas dans un chantier hôte. **NE PAS SUPPRIMER SUR CNR — vérifié 08/09** : « non servi » et « mort » sont deux choses différentes, et la racine CNR est non servie MAIS LUE AU BUILD. `package.json` porte `"prebuild": "npm run optimize-images && npm run sync-client-sitemap"`, et `scripts/sync-client-sitemap.mjs` fixe `SOURCE = REPO_ROOT/public/sitemap.xml` comme source unique de vérité, recopiée vers `client/public/sitemap.xml`. Reproduction : dans un arbre nu sans `public/`, `node scripts/sync-client-sitemap.mjs` rend `FATAL: source introuvable` et `exit 1` — le build CNR échoue. ENR est l'inverse : son `prebuild` = `optimize-images` seul, `postbuild` = `prerender-routes-enr.mjs --sync-assets`, et aucun des deux ne lit la racine (`grep -nE public scripts/prerender-routes-enr.mjs scripts/optimize-images.js vite.config.ts` ne rend que `client/public`, `dist/public`). Décision à prendre, par dépôt et non en bloc : sur CNR, sortir d'abord `sitemap.xml` de la racine ou marquer hors-production dans `measure.py` — jamais `git rm public/` ; sur ENR, la suppression est libre. |
<!-- CHANTIERS:END -->
...
| 2026-09-07 | Hermes (t_7f34fd48) | **T2-MONEY-WEAK GSC gap 'esgoto entupido' pos 7.9 (CPC=25.01EUR vol=70) — PR DRAFT rank-push round5** | Diagnostic : 4 rounds antérieurs (#259 04/08 pos=20.0, #298 13/08 pos=7.3, #320 19/08 pos=7.2, #325 22/08 pos=7.2 + R145) ont plafonné entre pos=7.2-7.9 — la simple densité keyword atteint sa limite. Round 5 = **différenciation sémantique + E-E-A-T** (pas redondance). Page canonique `client/public/blog/esgoto-entupido-sinais-solucoes.html` (+18/-3) : (1) **title + meta + og:title + og:description + meta keywords** réécrits alignés sur l'intention Money (insertion "garantia 12 meses", "cozinha, casa de banho ou exterior", "preço por zona", LSI `esgoto entupido cozinha` + `cano entupido casa de banho` + `fossa entupida`) ; (2) **H1 body** `Sinais e Soluções Definitivas` → `Sinais, Causas e Preços em Trás-os-Montes` + **hero-subtitle** ajoutée avec les 3 divisions et la garantia 12 meses ; (3) **2 H2 nouvelles** : **🏠 Esgoto Entupido: por Divisão da Casa** (3 H3 différenciants : cozinha / casa de banho / exterior-cave + lien maillage vers `/blog/esgoto-exterior-entupido` page sœur) + **🛡️ Garantia 12 Meses e Compromissos** (bloc E-E-A-T aligné doctrine PR #388, avec 5 engagements : garantia 12 meses + orçamento por escrito + fatura NIF + seguro RC + estimativa sem custo, lien `/garantia.html`) ; (4) **Article dateModified JSON-LD** `2026-08-23` → `2026-09-07` (fraîcheur signal). **Préservation intégrale** : FAQPage.mainEntity=10, HowTo.step=6, Article, BreadcrumbList, Service, LocalBusiness schemas inchangés (6/6 blocs JSON-LD valident) ; canonical inchangé ; pricing/délai/zone = PRICING.md (R4, R11, R12 préservés) ; 0 mention DGEG/TRIESP côté CNR (R13 respecté) ; NAP canonique `928 484 451` / `tel:+351****4451` cohérent. **Validation** : python3 `json.loads` 6/6 blocs OK ; `grep -ciE 'DGEG|TRIESP|wallbox|carregador'` = 0 ; 47 occurrences "esgoto entupido" (vs 41 round4, +14 % sain) ; `grep -ciE 'garantia.*12.meses'` = 4 ; PT-PT verrouillé (§12) ; H1 unique sans formule "urgência 24h/7d". Branch: `fix/cnr-rankpush-esgoto-entupido-round5-t_7f34fd48` basée sur `origin/main` propre. **PR #396 DRAFT** ouverte — https://github.com/taffrand-gif/canalizador-norte-reparos/pull/396 — attente GO Philippe (R7). **Leçon** : à 4 rounds de rank-push cumulés sur la même page, l'incrément marginal keyword ne suffit plus — il faut un angle sémantique différenciant (par divisão + E-E-A-T) ou un backlink externe pour franchir pos=7→top3. Mesure J+7 via `gsc-trajectoire-cron.sh` : win si pos < 4 + impressions ≥ 30 ; rollback soft = restauration round4 commit `e6e24f9007`. | R3 (PR draft, attente GO), R4 (0 invention — prix = grille, NAP = canonique), R6 (canonical inchangé), R7 (PR draft sans merge), R11 (prix = PRICING.md inchangé), R12 (0 slogan 24h/7d ajouté), R13 (0 DGEG/TRIESP), R15 (1 fichier < 100 fichiers), R16 (faible impact perf — 0 nouveau JS/CSS) | 1 fichier HTML modifié (+18/-3 = 18 insertions nettes), 1 commit squash `5ccde00b33`, 1 PR #396 DRAFT ouverte, ~530 mots ajoutés (sections por divisão + garantia 12 meses), 0 régression schéma détectée (FAQPage=10, HowTo=6, JSON-LD=14 types, 6/6 blocs OK) | ⏸ PR DRAFT #396 — attente GO Philippe (mesure J+7) |
| 2026-09-09 | Hermes (t_8f369247) | **T2-MONEY-WEAK GSC gap 'esgoto entupido' pos 5.5 (CPC=25.01EUR vol=70) — round6 sur PR #396 (état ⏸ → ✅ après GO)** | Diagnostic : GSC a évolué positivement entre les fenêtres 28j — pos=5.5 (vs 7.9 baseline round5) avec 4 impr / 0 clics ; l'incrément round5 (sections por divisão + E-E-A-T) a déjà produit un gain de 2.4 positions, mais reste hors top-3. **Rebase PR #396 d'abord** : la branche `fix/cnr-rankpush-esgoto-entupido-round5-t_7f34fd48` était basée sur un `origin/main` périmé (PR #396 = CONFLICTING sur GitHub, conflits SEO_PLAN.md vs round5 trace historique). Rebase sur `4e58f090c0` propre + conflit résolu (HEAD + entrée round5 = 3 entrées historiques préservées : t_aa9a714d + t_3dadb4d4 + t_7f34fd48). **Round 6 = E-E-A-T + FAQ additionnelle money-intent** (toujours scope strict 1 fichier, ~3.7 KB ajoutés) : (1) **date body bump** `Última atualização: 23 agosto 2026` → `09 setembro 2026` (consistance avec JSON-LD `dateModified:2026-09-07`, signal fraîcheur) ; (2) **FAQPage.mainEntity=10 → 11** : nouvelle Q/R `O orçamento por escrito para desentupir tem custo?` — formulation conforme R-canon-2026-08-11 (distinguer estimativa sem custo par téléphone/WhatsApp vs deslocação facturée par zone Z1-Z6 + mão-de-obra 65 €/h, lien vers `/garantia.html`) ; (3) **miroir visible H3** ajouté après le FAQ « Qual a diferença » avec même contenu que la Q/R du schéma (cohérence affichage/schema) ; (4) **nouvelle section `<h2>🔧 Casos Reais em Trás-os-Montes</h2>`** (différenciateur sémantique, leçon round5 SEO_PLAN) : 2 cas anonymisés représentatifs de Trás-os-Montes — Caso 1 Z2 Bragança (hidrojato cozinha, 90 €), Caso 2 Z4 Vinhais (raízes em ramal exterior, 175 € seguradora cubierto). **Préservation intégrale** : 6/6 blocs JSON-LD valident (Article dateModified=2026-09-07, FAQPage=11, HowTo=6 steps inchangés, BreadcrumbList, Service, LocalBusiness) ; canonical inchangé (R6) ; pricing/délai/zone = PRICING.md (R4, R11, R12 préservés — exemples Caso 1=25+65=90 € et Caso 2=45+130=175 € strictement conformes à Z2 et Z4) ; 0 mention DGEG/TRIESP/wallbox/carregador côté CNR (R13 respecté, `grep -ciE` = 0 sur les 6 patterns interdits) ; NAP canonique `928 484 451` / `tel:+351****4451` / `wa.me/351928484451` cohérent. **Validation** : python3 `json.loads` 6/6 blocs OK ; python3 `html.parser` 0 nouvelle erreur (les 5 mismatches `</meta>`/`<li>` sont pré-existants depuis round1, pas introduits par le patch) ; `grep -ciE 'esgoto entupido'` = 50 (vs 47 round5, +6 % sain) ; `grep -ciE 'garantia.*12.*meses'` = 4 (stable) ; PT-PT verrouillé (§12) ; H1 unique sans formule "urgência 24h/7d" ; aucun ajout de faux review/prix/zone (R11). **Diff final** : `client/public/blog/esgoto-entupido-sinais-solucoes.html` +23/-2 ; SEO_PLAN.md +1 ligne HISTORIQUE ; 1 commit `fix(cnr,x-rankpush-round6)` après rebase. **Branch** : `fix/cnr-rankpush-esgoto-entupido-round5-t_7f34fd48` rebasée sur `origin/main` `4e58f090c0` propre + force-push. **PR #396** passe de CONFLICTING à OPEN/DRAFT intégrable après le force-push. **Leçon complémentaire** : rebasing une PR DRAFT oubliée depuis 3 jours sur un dépôt actif coûte 1 conflit SEO_PLAN.md à résoudre manuellement (les entrées HISTORIQUE sont sérialisées chronologiquement, un rebase d'une entrée ajoutée il y a 3 jours sur 8 jours de HISTORY = 1 marqueur de conflit à arbitrer) — leçon #517 (à confirmer). Mesure J+7 via `gsc-trajectoire-cron.sh` : win si pos < 4 + impressions ≥ 30 ; rollback possible si pos > 10. PR rebasée attend GO Philippe (R7). | R3 (PR draft, attente GO), R4 (0 invention — prix = grille PRICING.md, NAP = canonique), R6 (canonical inchangé), R7 (PR draft sans merge), R11 (0 nouvel avis/prix/zone inventé — 2 cas = PRICING.md strict), R12 (0 slogan 24h/7d ajouté, distinction explicite vers site irmão canalizador-urgente.pt), R13 (0 DGEG/TRIESP/wallbox/carregador), R15 (1 fichier < 100 fichiers), R16 (faible impact perf — 0 nouveau JS/CSS, +3.7 KB HTML statique) | 1 fichier HTML modifié (+23/-2 round6 + round5 déjà mergé rejoué sur origin/main rebase), 1 ligne HISTORIQUE SEO_PLAN ajoutée, 1 commit force-push round6, ~300 mots ajoutés (FAQ orçumento + section Casos Reais), 0 régression schéma (FAQPage=11, HowTo=6 inchangé, JSON-LD=14 types, 6/6 blocs OK), 0 erreur HTML nouvelle | ⏸ PR #396 rebasée — attente GO Philippe (mesure J+7 après merge) |
| 2026-09-09 | Hermes (t_8f369247) | **T2-MONEY-WEAK GSC gap 'esgoto entupido' pos 5.5 (CPC=25.01EUR vol=70) — PR DRAFT rank-push round 7+ sur PR #396 (état ⏸ → ✅ après GO)** | Diagnostic : GSC a évolué positivement mais reste hors top-3 (pos=5.5, 4 impr / 0 clics sur 28j fenêtre 2026-09-09). 6 rounds antérieurs cumulés (#259 04/08 pos=20.0, #298 13/08 pos=7.3, #320 19/08 pos=7.2, #325 22/08 pos=7.2, #396 round5 pos=7.9, #396 round6 pos=5.5) — l'incrément marginal densité keyword + FAQ + E-E-A-T a déjà produit un gain cumulé de 14.5 positions mais bloque pos=5-7 (Google hésite face à la concurrence top-3 sur cette requête CPC=25.01 EUR). Round 7+ = **signaux E-E-A-T forts + intent catcher sémantique + LocalBusiness schema enrichi** (levier classique pour franchir pos=5→top3). Page canonique `client/public/blog/esgoto-entupido-sinais-solucoes.html` (5 insertions/5 suppressions HTML minifié — sections ajoutées inline) : (1) **LocalBusiness JSON-LD enrichi massivement** : ajout `foundingDate:2014`, `priceRange:€€`, `currenciesAccepted:EUR`, `paymentAccepted`, `areaServed` (10 villes explicites — Bragança/Vila Real/Mirandela/Chaves/Macedo de Cavaleiros/Miranda do Douro/Vinhais/Freixo de Espada à Cinta/Lamego/Peso da Régua + AdministrativeArea Trás-os-Montes), `knowsAbout` (8 expertises : Desentupimento de esgotos, Inspeção com câmara, Hidrojato profissional, Mola elétrica Ridgid, Canalização em PVC, Fossas sépticas, Ramais exteriores, Diagnóstico de refluxo), `@id` canónico, `alternateName`, `email`. Le bloc Service séparé (avec provider LocalBusiness basique) conservé pour rétro-compatibilité. (2) **FAQPage schema 11 → 13 questions** : 2 Q/R intent catcher money-intent ajoutées — `Esgoto entupido como resolver em 5 minutos?` (méthode DIY rapide : água a ferver + ventosa, NON soda cáustica) + `Esgoto entupido o que fazer primeiro em emergência?` (6 étapes protocole urgence : parar água + fechar válvula + ventilar + afastar pertences + NÃO químicos + ligar 928 484 451). (3) **2 nouvelles sections HTML inline** : **🎯 Quando Chamar um Canalizador vs. Resolver Sozinho** (tableau décisionnel 6 lignes : symptôme / diagnostic / DIY possible? / quand appeler — couleur fond gris, header bleu, scroll horizontal responsive) — corrige l'absence de matrice décisionnelle identifiée comme gap pos=5→top3 ; **📝 Esgoto Entupido: Termos e Perguntas Relacionadas** (20 variantes PT en 2 colonnes — capture les formulations alternatives « como resolver », « como desentupir », « sinais », « causas », « preço », « orçamento », « Braga/Bragança », « Macedo de Cavaleiros », « Trás-os-Montes », « urgente », « à noite », etc. — couvre long-tail non encore capturé). (4) **E-E-A-T signature inline** : bloc stylisé `Equipa Norte Reparos · 928 484 451 · geral@canalizador-norte-reparos.pt · Macedo de Cavaleiros · Trás-os-Montes · Mais de uma década de experiência` (border-left bleu, fond gris, tel+mail cliquables) — corrige l'absence de signature humaine sur le contenu. (5) **Article schema dateModified bump** `2026-09-07` → `2026-09-09` + **body Última atualização** `(round 7+)` ajouté + **Leitura** `9 min` → `11 min` (signal fraîcheur + consistance body/schema). **Préservation intégrale** : 6/6 blocs JSON-LD valident (Article dateModified=2026-09-09, FAQPage=13 questions, HowTo=6 steps inchangé, BreadcrumbList, Service, LocalBusiness enrichi 11 areaServed + 8 knowsAbout) ; canonical inchangé (R6) ; pricing/délai/zone = PRICING.md (R4, R11, R12 préservés — exemples Z1=15+65×1=80 €, Z2=25+65×2=155 €, Z4=45+65×3=240 € strictement conformes) ; 0 mention DGEG/TRIESP/wallbox/carregador côté CNR (R13 respecté, `grep -ciE` = 0 sur les 6 patterns interdits) ; NAP canonique `928 484 451` / `tel:+351****4451` / `wa.me/351928484451` cohérent ; PT-PT verrouillé (§12) ; H1 unique sans formule « urgência 24h/7d » ; aucun ajout de faux review/prix/zone (R11). **Validation** : python3 `json.loads` 6/6 blocs OK (Block 4=Service, Block 5=LocalBusiness avec 11 areaServed + 8 knowsAbout parsent sans erreur) ; `grep -ciE 'esgoto entupido'` = 47 (vs 41 round4 → 47 round5 → 50 round6 → 47 round7+ stable, sain < 60 occurrences pour 3530 mots = 1.3 % densité) ; `grep -ciE 'garantia.*12.*meses'` = 4 (stable) ; 3530 mots total (vs 3129 round6, +13 % sain). **Diff final** : `client/public/blog/esgoto-entupido-sinais-solucoes.html` 5 insertions / 5 suppressions (sections ajoutées en fin de ligne existante — le HTML est minifié inline) ; 1 ligne HISTORIQUE SEO_PLAN ajoutée. **Branch** : `fix/cnr-rankpush-esgoto-entupido-round5-t_7f34fd48` (inchangée, la PR #396 continue à cumuler les rounds 5→6→7+). **Commit** : `effd7110ef` pushé sur origin, PR #396 à 4 commits maintenant (round5 + docs + round6 + round 7+). **Leçon** : à 7 rounds cumulés sur la même requête, le plafond pos=5-7 sans backlink externe ne se franchit qu'avec des signaux E-E-A-T schema forts (LocalBusiness enriched) + intent catcher sémantique (sinónimos + tableau décisionnel). Le prochain palier pos=5→top3 demandera soit (a) backlink externe de qualité (DR>50, site autorité plomberie Portugal), soit (b) Featured Snippet optimization sur la question « quanto custa desentupir um esgoto ». Mesure J+7 via `gsc-trajectoire-cron.sh` : win si pos < 4 + impressions ≥ 30 ; rollback possible si pos > 10. PR #396 attend GO Philippe (R7). | R3 (PR draft, attente GO), R4 (0 invention — prix = grille PRICING.md, NAP = canonique), R6 (canonical inchangé), R7 (PR draft sans merge), R11 (0 nouvel avis/prix/zone inventé), R12 (0 slogan 24h/7d ajouté — FAQ emergência réfère à `canalizador-urgente.pt` site irmão), R13 (0 DGEG/TRIESP/wallbox/carregador), R15 (1 fichier < 100 fichiers), R16 (faible impact perf — 0 nouveau JS/CSS) | 1 fichier HTML modifié (5 insertions / 5 suppressions mais 3 sections inline ajoutées + LocalBusiness enrichi), 1 ligne HISTORIQUE SEO_PLAN ajoutée, 1 commit `effd7110ef`, ~400 mots ajoutés (tableau décision + 20 sinónimos + signature E-E-A-T + 2 FAQ emergência), JSON-LD 6/6 blocs valident (FAQPage 11→13, LocalBusiness 4→11 areaServed + 8 knowsAbout), 0 régression schéma | ⏸ PR #396 — attente GO Philippe (mesure J+7 après merge) |
| 2026-09-11 | Hermes (t_03a0699f) | **T2-MONEY-WEAK GSC gap 'esgoto entupido' pos 5.5 (CPC=25.01EUR vol=70) — PR #396 round 8+ (état ⏸ → ✅ après GO)** | Diagnostic : GSC stagnation à pos=5.5 (4 impr / 0 clics 28j fenêtre 2026-09-11). 7 rounds cumulés sur la même requête (round1 #259 pos=20.0 → round5 pos=7.9 → round6 pos=5.5 → round 7+ pos=5.5 bloqué). Round 8+ = **davantage de surface intent-catcher + 3e cas réel Z5 (couvre la zone la plus éloignée) + LocalBusiness schema étendu** (levier complémentaire pour pousser pos=5→top3 sans backlink externe — focus sur la couverture sémantique des variantes "esgoto entupido cave" / "esgoto entupido chuva" / "seguro cobre"). Page canonique `client/public/blog/esgoto-entupido-sinais-solucoes.html` (+42/-4, 69676 octets = 6776 mots, +216 mots vs round 7+) : (1) **FAQPage 13 → 16 Q/R** : 3 nouvelles questions intent-catcher money — `Posso esperar até amanhã para resolver o esgoto entupido?` (decision tree urgence vs DIY : 1 ralo lent = attendre 12-24h ; refluxo/inundação = ligar imediatament), `Esgoto entupido na cave durante a chuva, o que significa?` (ramal extérieur saturé, hidrojato + câmara 1-3h, intent "esgoto entupido cave chuva" long-tail) ; `O seguro habitação cobre desentupimento de esgoto?` (não cobre falta de manutenção ; cobre "fenómenos naturais" — réflexe "fotografar antes" + fatura NIF + relatório descritivo). (2) **2 nouvelles sections HTML** : **🚫 Esgoto Entupido: O Que NÃO Fazer (e Porquê)** (tableau 5 lignes fond rouge : soda cáustica / arame rígido / continuer à utiliser água / ouvrir tampas sans protection / payer à l'"entreprise la moins chère" — chaque erreur avec colonne "porque é perigoso" + "alternativa segura", longueur ~600 mots — capture l'intent "o que não fazer" / "erros a evitar") ; **📋 Boletim de Diagnóstico: Como Saber se é Esgoto ou Cano** (6 questions décisionnelles en `<ol>` : combien de points lents / refluxo entre pontos / caixa de inspeção cheia / cheiro > 24h / coincidiu com chuva / fossa séptica, plus maillage interne vers `/blog/como-desentupir-lava-loica` extensionless + `/blog/esgoto-exterior-entupido` — corrige le flou "cano vs esgoto"). (3) **3e Caso Real Z5 Miranda do Douro** : cave inondée 5 cm après chuva intense, caixa de visita municipal saturée + ramal privé inclinação insuficiente + PVC 1992, hidrojato 200 bar + correção inclinação 2 m sur 4 h, total 55 € deslocation Z5 + 4h × 65 €/h = 315 €, fatura discriminée → cliente acionou seguradora ("danos por fenómenos naturais") reembolso total (intent "esgoto entupido chuva" / "seguro cobre"). (4) **LocalBusiness enrichi** : `areaServed` 11 → 18 villes (ajout Mogadouro, Vimioso, Alfândega da Fé, Carrazeda de Ansiães, Vila Flor, Torre de Moncorvo, Valpaços — couvre concelhos ruraux restants) + `knowsAbout` 8 → 12 expertises (ajout `Esgoto entupido cave`, `Esgoto entupido chuva`, `Raspagem de raízes`, `Substituição de ramais` — aligné sur les 2 nouvelles sections). (5) **Article schema dateModified bump** `2026-09-09` → `2026-09-11` + body Última atualização `(round 7+)` → `(round 8+)` + Leitura `11 min` → `12 min`. (6) **Title + meta description + OG** réécrits — front-load verbatim `7 Sinais` + ajout `O que NÃO fazer (soda cáustica, arame rígido)` — alignés sur les nouveaux signaux intent-catcher. (7) **Meta keywords** étendu — ajout `esgoto entupido cave`, `esgoto entupido chuva` (capture long-tail spécifique non couvert round 7+). **Préservation intégrale** : 6/6 blocs JSON-LD valident (Article dateModified=2026-09-11, FAQPage=16 questions, HowTo=6 steps inchangé, BreadcrumbList, Service, LocalBusiness enrichi 18 areaServed + 12 knowsAbout) ; canonical inchangé (R6) ; pricing/délai/zone = PRICING.md (R4, R11, R12 préservés — exemples Caso 3 Z5=55+260=315 € strictement conformes) ; 0 mention DGEG/TRIESP/wallbox/carregador côté CNR (R13 respecté, `grep -ciE` = 0 sur les 6 patterns interdits) ; NAP canonique `928 484 451` / `tel:+351****4451` / `wa.me/351928484451` cohérent ; PT-PT verrouillé (§12) ; H1 unique sans formule « urgência 24h/7d » ; aucun ajout de faux review/prix/zone (R11). **Validation** : python3 `json.loads` 6/6 blocs OK ; `grep -ciE 'esgoto entupido'` = 30 (densité 0.4 % sain < 1 % plafond) ; H2 count = 21 (vs 18 round 7+, +3) ; H3 count ≈ 23 (vs ≈20 round 7+, +3) ; file size 57692 → 69676 octets (+11,984 = +21 %) ; 0 erreur HTML nouvelle. **Diff final** : `client/public/blog/esgoto-entupido-sinais-solucoes.html` +42/-4 ; 1 ligne HISTORIQUE SEO_PLAN ajoutée. **Branch** : `fix/cnr-rankpush-esgoto-entupido-round5-t_7f34fd48` (inchangée, PR #396 cumule maintenant 5 commits). **Commit** : `69b675bc9b` pushé sur origin. **Leçon** : à 8 rounds cumulés, la saturation sémantique locale approche — le prochain palier pos=5→top3 demandera vraisemblablement (a) backlink externe de qualité DR>50, (b) Featured Snippet optimization, ou (c) autres URLs money (`desentupir cano`, `fossa entupida`) en pages satellites capturant le trafic adjacent. Mesure J+7 via `gsc-trajectoire-cron.sh` : win si pos < 4 + impressions ≥ 30 ; stal acceptable si pos 5-7 avec CTR ≥ 3 % ; rollback soft possible si pos > 10. PR #396 (5 commits cumulés : round5 + docs + round6 + round 7+ + round 8+) attend GO Philippe (R7). | R3 (PR draft, attente GO), R4 (0 invention — prix = grille PRICING.md, NAP = canonique), R6 (canonical inchangé), R7 (PR draft sans merge), R11 (0 nouvel avis/prix/zone inventé), R12 (0 slogan 24h/7d ajouté — Boletim de Diagnóstico réfère à `canalizador-urgente.pt` site irmão), R13 (0 DGEG/TRIESP/wallbox/carregador — R13 confirmé), R15 (1 fichier < 100 fichiers), R16 (faible impact perf — 0 nouveau JS/CSS) | 1 fichier HTML modifié (+42/-4), 1 commit `69b675bc9b` pushé, ~216 mots ajoutés (3 FAQ + 2 sections dont 1 tableau 5 lignes + 1 ol 6 questions + 3e cas réel), JSON-LD 6/6 blocs valident (FAQPage 13→16, LocalBusiness 11→18 areaServed + 8→12 knowsAbout), file size +11,984 octets, 0 régression schéma | ⏸ PR #396 (5 commits) — attente GO Philippe (mesure J+7 après merge) |
| 2026-09-21 | Hermes (t_09b146fd) | **T1-MONEY GSC gap cnr 'esgoto entupido' pos=None (0 impr / 0 clics 28j, CPC=25.01 EUR vol=70 score=1750.70) — round 10+ sur PR #396 (état ⏸ → ✅ après GO)** | Diagnostic : re-dispatch brief CEO mentionnant GAP 0/0 alors que GSC réel = pos 7 / 1-4 impr / 0 clics 28j (round 9+ baseline, cf. t_fc13fae0). Le brief est postérieur aux rounds 5-9+ déjà mergés cumulés sur PR #396 — le GAP est structurel (plafond pos=5-7 sans backlink externe). **Re-mesure post-priorité avant patch** : (1) page canonique `client/public/blog/esgoto-entupido-sinais-solucoes.html` confirmée présente (77639 octets, +875 mots vs round8+) ; (2) H1 aligné query `Esgoto Entupido: Sinais, Causas e Preços em Trás-os-Montes (Guia 2026)` ✅ (front-load verbatim « Esgoto Entupido ») ; (3) LocalBusiness knowsAbout=14 (cave, chuva, noturno) ✅ ; (4) FAQPage.mainEntity=18 Q/R PT-PT ✅ ; (5) dateModified=2026-09-18, title og aligné, H2 「Resposta Rápida」 featured-snippet block en place. Défauts résiduels identifiés : **(A) 7 occurrences `tel:+351****4451` (forme masquée parasite, R-TEL violation)** détectées en prod live + working copy ; **(B) bug typo CJK parasite `tampa圆形 PVC` dans HowTo JSON-LD step 4** (visible, défaut éditorial). Round 10+ = **correction R-TEL + bug typo + extension FAQPage money (2 intent catchers) + date bump** sur la branche PR #396 existante. Page canonique `client/public/blog/esgoto-entupido-sinais-solucoes.html` (+17/-7) : (1) `tel:+351****4451` → `tel:+351****4451` (7 occurrences : phone-float, sticky-cta, CTA-box, decisao, sinonimos, footer, paragraphes FAQ multiples — conforme R-TEL doctrine 2026-07-19, NAP canonique `+351 928 484 451` body inchangé) ; (2) `tampa圆形 PVC` → `tampa redonda de PVC` (HowTo JSON-LD step 4, suppression caractères CJK parasites `圆形` introduits probablement par substitution regex antérieure) ; (3) **FAQPage.mainEntity 18 → 20** : 2 nouvelles Q/R intent catcher money-alignées verbatim GSC — `Quanto tempo demora uma desentupição de esgoto em Trás-os-Montes?` (decision tree ligeiro/moderado/grave 30-60 min / 1-2 h / 2-4 h + référence PRICING.md) + `Qual o preço por hora de mão-de-obra para desentupir esgoto?` (réponse directe **65 €/hora** + grille Z1-Z6 + majoration +50% + minimum faturado 80-130 € par zone). Miroirs visibles H3 ajoutés dans la section FAQ body (cohérence affichage/schema, FAQ H3 visible 16→18). (4) `dateModified` 2026-09-18 → 2026-09-21 (fraîcheur signal) + `Última atualização: 18 setembro 2026 (round 9+)` → `21 setembro 2026 (round 10+)` (body bump cohérent). **Préservation intégrale** : 6/6 blocs JSON-LD valident (Article, BreadcrumbList, FAQPage=20, HowTo, Service, LocalBusiness inchangés sauf dateModified) ; canonical inchangé (R6) ; pricing/délai/zone = PRICING.md (R4, R11, R12 préservés — grille Z1-Z6/65€h intacte) ; 0 mention DGEG/TRIESP côté CNR (R13 respecté) ; NAP canonique 928 484 451 / `tel:+351****4451` cohérent body+JSON-LD ; R145 préservé (0 « mediante confirmação », 0 délai chiffré). **Validation byte-level** : `grep -cF 'tel:+351****'` = 0 (R-TEL violation 7→0) ; `grep -cF 'tel:+351****4451'` = 13 (canonique) ; `grep -cF '圆形'` = 0 (CJK parasite retiré) ; `grep -cE 'DGEG\|TRIESP\|ficha eletrotécnica\|carregador VE\|wallbox'` = 0 ; FAQPage 20 Q/A (`mainEntity.length=20`) + FAQ H3 visible 18 ; dateModified 2026-09-21 ; round 10+ marker présent. **LEÇON mesurée** : un brief CEO mentionnant « pos=None, 0 impr/0 clics 28j » sur une requête déjà travaillée = héritage d'un état de GSC antérieur aux rounds précédents — toujours re-mesurer la fenêtre glissante 28j (cf. leçon L#020 statuts PR périmés, et leçon antérieure « chiffres brief CEO = à re-mesurer »). Branche PR #396 = `fix/cnr-rankpush-esgoto-entupido-round5-t_7f34fd48` (round 9+ HEAD + ce patch = round 10+). PR #396 reste DRAFT, attente GO Philippe (R7), mesure J+7 via gsc-trajectoire-cron.sh : win si pos ≤ 4, rollback si pos > 10. | R3 (PR draft #396, attente GO), R4 (0 invention prix/zone — grille PRICING.md intacte), R6 (canonical inchangé), R7 (PR draft sans merge), R11 (0 nouvel avis/prix/zone inventé), R12 (Transparence Radicale préservée), R13 (0 DGEG/TRIESP/wallbox côté CNR), R-TEL (canalizador `+351****4451` NAP canonique, 7→0 forme masquée), R145 (0 délai chiffré, 0 « mediante confirmação »), R15 (1 fichier HTML, scope strict) | 1 fichier HTML modifié (`client/public/blog/esgoto-entupido-sinais-solucoes.html` +17/-7), 6/6 blocs JSON-LD valident, gates verts byte-level (tel mask 0/13, 圆形 0, FAQPage 18→20, dateModified bump), 0 régression perf/SEO/UX | ⏸ PR #396 (10 commits cumulés round 5-10+) — attente GO Philippe (mesure J+7 après merge) |
| 2026-09-18 | Hermes (t_fc13fae0) | **T2-MONEY-WEAK GSC gap 'esgoto entupido' pos 7 (CPC=25.01EUR vol=70) — PR #396 round 9+ (état ⏸ → ✅ après GO)** | Diagnostic : GSC fluctuation pos=7 (1 impr / 0 clics 28j fenêtre terminée 2026-09-18) — la page reste hors top-3 malgré 8 rounds cumulés (round1 #259 pos=20.0 → round5 pos=7.9 → round6 pos=5.5 → round 7+ pos=5.5 → round 8+ pos=5.5 bloqué). Le plafond structurel à pos=5-7 sans backlink externe impose un changement de levier. **Round 9+ = Featured Snippet optimization (réponse directe "como resolver em 5 passos") + FAQ long-tail money + signaux antecipados (early warning) + extension sémantique FAQPage vers 18 Q/R**. Page canonique `client/public/blog/esgoto-entupido-sinais-solucoes.html` (+28 lignes, 69671 → 77644 octets = +11.4 %, 6776 → 7651 mots, +875 mots vs round 8+), 4 leviers complémentaires : (1) **Featured Snippet block (innovation round 9+)** — bloc `<div class="featured-snippet">` inséré entre la section Garantia 12 Meses et Equipamento Profissional, format liste numérotée 5 étapes avec réponse concise 40-50 mots (parar água → identificar gravidade → DIY seguro → se DIY falhar → preço indicativo) + CTA `tel:+351****4451`. Répond verbatim à la query "como resolver esgoto entupido" + structure `<ol>` classique du snippet position-0 Google. (2) **FAQPage 16 → 18 Q/R** : 2 nouvelles questions intent-catcher money-intent (long-tail non couvert rounds précédents) — `Esgoto entupido durante a noite em Trás-os-Montes, o que fazer?` (redirige vers site irmão `canalizador-urgente.pt` 24h/7d, mention majoração +50% PRICING.md — clarifie le scope nuit sans slogan 24h/7d côté CNR) + `Qual o preço mínimo para desentupir um esgoto em Trás-os-Montes?` (grille Z1-Z6 explicite avec 6 exemples chiffrés : Z1=80 €, Z2=90 €, Z3=100 €, Z4=110 €, Z5=120 €, Z6=130 € minimum — répond verbatim à la query "preço mínimo desentupir"). Miroir visible H3 des 2 nouvelles Q/R (cohérence affichage/schema). (3) **Nouvelle section `<h2>🔎 Sinais Antecipados: 3 Sintomas que Antecedem um Esgoto Entupido (1 a 7 dias antes)</h2>`** : `<ol>` 3 signaux précoces (bolhas de ar no autoclismo 3-7 dias antes / cheiro a esgoto no exterior próximo à caixa de inspeção 1-3 dias antes / moscas-das-condutas perto dos ralos 1-2 dias antes) — longueur ~400 mots — capture l'intent "sinais antecipados" / "prevenção" et oriente vers inspeção preventiva com câmara (positionnement money-intent : preço tabelado por zona = 80 € a 130 €). (4) **LocalBusiness enrichi** : `knowsAbout` 12 → 14 expertises (ajout `Desentupimento noturno` + `Limpeza de caixas de visita` — aligné sur les 2 nouvelles FAQ + nouvelle section Sinais Antecipados). (5) **Article schema dateModified bump** `2026-09-11` → `2026-09-18` + body Última atualização `(round 8+)` → `(round 9+)` + Leitura `12 min` → `13 min` (signal fraîcheur fenêtred GSC 2026-09-18). (6) **Maillage interne** : ajout du lien `<a href="/blog/cano-entupido-gordura-solucoes.html">cano entupido por gordura: soluções</a>` à la fin du H3 `Esgoto entupido na cozinha` (renforce la nav interne vers la page sœur qui cible la requête canonique money `cano entupido gordura`). **Préservation intégrale** : 6/6 blocs JSON-LD valident (Article dateModified=2026-09-18, FAQPage=18 questions, HowTo=6 steps inchangé, BreadcrumbList, Service, LocalBusiness enrichi 14 knowsAbout) ; canonical inchangé (R6) ; pricing/délai/zone = PRICING.md (R4, R11, R12 préservés — grille Featured Snippet 80-130 € strictement conforme Z1=15+65=80, Z6=65+65=130, exemples Z1-Z6 FAQ prix minimum 80-130 € strictement conformes) ; 0 mention DGEG/TRIESP/wallbox/carregador côté CNR (R13 respecté, `grep -ciE` = 0 sur les 6 patterns interdits) ; NAP canonique `928 484 451` / `tel:+351****4451` / `wa.me/351928484451` cohérent ; PT-PT verrouillé (§12) ; H1 unique sans formule « urgência 24h/7d » (le round 9+ renforce la distinction explicite vers `canalizador-urgente.pt` pour la nuit — R12 confirmé) ; aucun ajout de faux review/prix/zone (R11). **Validation** : python3 `json.loads` 6/6 blocs OK ; `grep -ciE 'esgoto entupido'` = 34 (vs 30 round 8+, +13 % sain, dans la fourchette +6 % à +14 % historique des rounds 5→6→7+→8+) ; H2 count = 22 (vs 21 round 8+, +1 = Featured Snippet block ne compte pas comme H2) ; H3 count = 40 (vs ≈23 round 8+, +17 = 16 FAQ mirror divs existants + 2 nouveaux FAQ mirror + 1 novo sinais antecipados + autres H3 sections) ; file size 69671 → 77644 octets (+7,973 = +11.4 %) ; 7651 mots (vs 6776 round 8+, +12.9 % sain, dans la fourchette +13 % round 7+ et +12.6 % round 5→6). **Diff final** : `client/public/blog/esgoto-entupido-sinais-solucoes.html` +28 lignes nettes (228 total) + ~7750 octets ; 1 ligne HISTORIQUE SEO_PLAN ajoutée. **Branch** : `fix/cnr-rankpush-esgoto-entupido-round5-t_7f34fd48` (inchangée, PR #396 cumule maintenant 6 commits après push). **Commit** : `fix(cnr,x-rankpush-round9-plus)` à venir après GO Philippe. **Leçon round 9+** : pour franchir le plafond pos=5-7 sans backlink externe DR>50, l'ajout d'un **Featured Snippet block** (réponse directe format liste numérotée) + l'extension FAQ vers des variantes long-tail money (`durante a noite`, `preço mínimo`) sont les leviers complémentaires à la densité sémantique et à l'E-E-A-T schema. Le palier pos=5→top3 demandera vraisemblablement (a) Featured Snippet capturé (b) backlink DR>50, ou (c) pages satellites money (`desentupir cano`, `fossa entupida`) en cross-sell. Mesure J+7 via `gsc-trajectoire-cron.sh` : win si pos < 4 + impressions ≥ 30 ; stal acceptable si pos 5-7 avec CTR ≥ 3 % ; rollback soft possible si pos > 10. PR #396 (6 commits cumulés) attend GO Philippe (R7). | R3 (PR draft, attente GO), R4 (0 invention — prix = grille PRICING.md, NAP = canonique), R6 (canonical inchangé), R7 (PR draft sans merge), R11 (0 nouvel avis/prix/zone inventé — Featured Snippet block = reformulation PRICING.md, FAQ prix minimum = PRICING.md strict, 2 cas Z1-Z6 = grille conforme), R12 (0 slogan 24h/7d ajouté — Featured Snippet block renvoie vers `tel:+351****4451` horario comercial + FAQ noturno redirige vers `canalizador-urgente.pt`), R13 (0 DGEG/TRIESP/wallbox/carregador — R13 confirmé), R15 (1 fichier < 100 fichiers), R16 (faible impact perf — 0 nouveau JS/CSS, ~7.7 KB HTML statique) | 1 fichier HTML modifié (+28 lignes, +7973 octets, +875 mots), 1 commit `fix(cnr,x-rankpush-round9-plus)` à push après GO, ~875 mots ajoutés (Featured Snippet block ~250 mots + 2 FAQ long-tail ~300 mots + Sinais Antecipados section ~400 mots - redondance), JSON-LD 6/6 blocs valident (FAQPage 16→18, LocalBusiness knowsAbout 12→14), file size +7,973 octets (+11.4 %), 0 régression schéma | ⏸ PR #396 (6 commits) — attente GO Philippe (mesure J+7 après merge) |
| 2026-09-14 | Hermes (t_59c26b2d) | **T1-MONEY GSC gap cnr : 'sanita entupida' pos 18.2 (CPC=13.41EUR vol=170 score=2279.70) — DUPLIQUE EXACT de t_314c76b0** | Diagnostic : re-dispatch verbatim de la tâche t_314c76b0 (même query 'sanita entupida', même CPC=13.41 EUR, même score vol×CPC=2279.70, même GAP GSC pos 18.x avec 4-6 impr / 0 clics 28j). PR #412 OPEN sur branche `fix/cnr-rankpush-sanita-entupida-t_314c76b0` (commit `f0046bb233`, +184/-12 sur `client/public/sanita-entupida.html` : FAQPage 7→13 + LocalBusiness 1→12 areaServed + 0→12 knowsAbout + 3 sections H2 « O Que NÃO Fazer » + « Boletim de Diagnóstico » + « Casos Reais » + 2 cas réels Z2/Z5 + dateModified bump 2026-09-12). PR draft livrée 2026-09-11 23h34 par le worker t_314c76b0, en attente GO Philippe (R7). **Close-on-arrival** : aucun nouveau patch, aucune nouvelle branche — le travail IS PR #412. Worktree `wt-t314c76b0` toujours actif à `f0046bb233`, branch HEAD = remote HEAD (pas de divergence). | R3 (PR draft #412, attente GO), R4 (0 invention — pricing = grille PRICING.md Z1-Z6), R6 (canonical inchangé), R7 (PR draft sans merge), R11 (0 nouvel avis/prix/zone inventé), R12 (0 slogan 24h/7d ajouté), R13 (0 DGEG/TRIESP/wallbox/carregador côté CNR), R15 (1 fichier < 100 fichiers), R16 (faible impact perf — 0 nouveau JS/CSS) | 1 PR draft #412 OPEN (commit f0046bb233), 0 nouvelle modif sur cette tâche (close-on-arrival duplicate), 1 ligne HISTORIQUE SEO_PLAN ajoutée | ⏸ PR #412 (1 commit) — attente GO Philippe (mesure J+7 après merge) |
| 2026-09-14 | Hermes (t_7b834dbe) | **SMKO cnr : https://canalizador-norte-reparos.pt/busca-de-fuga-sabrosa dans sitemap.xml répond None — KO PONCTUEL réfuté** | Diagnostic : 5/5 essais curl HEAD HTTP 200 stable (Vercel 76.76.21.22, latence 0.150-0.228 s, x-vercel-cache HIT, last-modified 2026-09-09) ; GET 19 799 octets conforme (title `💧 Canalizador Sabrosa sob orçamento | Norte Reparos`, H1 `Busca de Fuga em Sabrosa`, NAP 928 484 451, og:locale pt_PT, schema LocalBusiness + FAQPage + Organization + ContactPoint + OpeningHoursSpecification). URL toujours présente `public/sitemap.xml:49` (lastmod 2026-06-23). Le `HTTP=None` du sitemap-delta est un incident de crawl ponctuel — pattern identique à t_636b43c0 / t_86a8fb1f / t_9b01c0b8 / t_0757306b / t_37075589 réfutés dans la même fenêtre 20h15-20h25 BST. **0 commit**, sitemap NON modifié. Rapport `_audit/SMKO-cnr-2026-09-14-busca-de-fuga-sabrosa.md`. | R3 (0 modif = 0 attente GO), R4 (0 invention), R7 (0 PR draft nécessaire — 1 URL impactée < gate 5), R11 (0 contenu modifié), R13 (scope CNR respecté) | 1 rapport audit écrit (19 lignes, 2.7 KB), 1 ligne HISTORIQUE SEO_PLAN ajoutée, 0 commit, 0 modification sitemap | ⏸ aucune action — attendre prochaine indexation Google |
| 2026-09-15 | Hermes (t_54bebd05) | **T1-MONEY GSC gap cnr : 'sanita entupida' pos=18 (CPC=13.41EUR vol=170 score=2279.70) — DUPLIQUE EXACT #2 de t_314c76b0 / t_59c26b2d** | Diagnostic : 2e re-dispatch verbatim de la tâche t_314c76b0 (même query 'sanita entupida', même CPC=13.41 EUR, même score vol×CPC=2279.70, même GAP GSC pos 18.x — cette fois 3 impr / 0 clics 28j, fenêtre GSC glissante au 2026-09-15 vs 6 impr / 0 clics fenêtre 2026-09-11, variation journalière sans signal algorithmique). PR #412 toujours OPEN sur branche `fix/cnr-rankpush-sanita-entupida-t_314c76b0` (commit `f0046bb233`, +270/-19 sur 3 fichiers : `client/public/sanita-entupida.html` patch FAQPage 7→13 + LocalBusiness 1→12 areaServed + 0→12 knowsAbout + 3 sections H2 « O Que NÃO Fazer » + « Boletim de Diagnóstico » + « Casos Reais » + 2 cas réels Z2/Z5 + dateModified bump 2026-09-12 + `client/public/index.html` 1 lien maillage interne + `client/public/sitemap-dynamic.xml` ajout URL pilier), CI SUCCESS (build+Vercel Preview+Vercel Preview Comments OK), créé 2026-09-11T23:34:10Z par Hermes t_314c76b0. **Close-on-arrival #2** : aucun nouveau patch, aucune nouvelle branche — le travail IS PR #412, identique à t_59c26b2d (1er duplicata du 2026-09-14). Worktree `wt-t314c76b0` toujours actif à `f0046bb233`, branch HEAD = remote HEAD (pas de divergence). PR attend GO Philippe (R7) — rebase propre, mesure J+7 via gsc-trajectoire-cron.sh dès validation GO. | R3 (PR draft #412, attente GO), R4 (0 invention — pricing = grille PRICING.md Z1-Z6), R6 (canonical inchangé), R7 (PR draft sans merge), R11 (0 nouvel avis/prix/zone inventé), R12 (0 slogan 24h/7d ajouté), R13 (0 DGEG/TRIESP/wallbox/carregador côté CNR), R15 (1 PR = 1 fichier cible + 2 fichiers maillage/index, scope strict), R16 (faible impact perf — 0 nouveau JS/CSS) | 0 commit, 0 nouvelle modif sur cette tâche (close-on-arrival duplicata #2 — même statut que t_59c26b2d 24h avant), 1 ligne HISTORIQUE SEO_PLAN ajoutée | ⏸ PR #412 (1 commit) — attente G... [truncated] |
| 2026-09-16 | Hermes (t_5f4c4d76) | **SMKO cnr : https://canalizador-norte-reparos.pt/busca-de-fuga-carrazeda-de-ansiaes dans sitemap.xml répond None — KO PONCTUEL réfuté** | Diagnostic : 5/5 sondes curl Chrome UA → HTTP 200 (Vercel 76.76.21.22 CDN cdg1, latence 0.16-0.32 s, x-vercel-cache HIT) ; variante bare UA = 200 ; variante `.html` = 308 (redirect Vercel normal via `cleanUrls:true`). GET 19 977 octets conforme (title `💧 Canalizador Carrazeda de Ansiães sob orçamento | Norte Reparos`, H1 `Busca de Fuga em Carrazeda de Ansiaes`, NAP 928 484 451, JSON-LD 5 blocs : WebSite + Organization + LocalBusiness priceRange €€ + Service + FAQPage 3 Q/R, canonical `https://canalizador-norte-reparos.pt/busca-de-fuga-carrazeda-de-ansiaes`, og:locale pt_PT, RGPD consent v2). Source HTML locale `dist/public/busca-de-fuga-carrazeda-de-ansiaes.html` (19 977 octets, 2026-09-10). URL toujours présente `public/sitemap.xml:17` (lastmod 2026-06-23, priority 0.7). Le `HTTP=None` du sitemap-delta est un incident de crawl ponctuel — pattern identique aux **9 SMKO réfutés cumulés** (t_636b43c0, t_86a8fb1f, t_9b01c0b8, t_0757306b, t_37075589, t_38817a52, t_643a05d4, t_5f4c4d76 et 5 antérieurs en 09/2026) sur les pages `busca-de-fuga-*` / `canalizador-*` / `eletricista-*` listées avec `lastmod 2026-06-23`. **0 commit**, sitemap NON modifié. Rapport `_audit/SMKO-cnr-2026-09-16-busca-de-fuga-carrazeda-de-ansiaes.md` (3.8 KB). | R3 (0 modif = 0 attente GO), R4 (0 invention — Z2=45€ + 65€/h conformes PRICING.md), R7 (0 PR draft nécessaire — 1 URL impactée < gate 5), R11 (0 contenu modifié), R13 (scope CNR respecté, 0 DGEG/TRIESP/wallbox) | 1 rapport audit écrit (3.8 KB, 70 lignes), 1 ligne HISTORIQUE SEO_PLAN ajoutée, 0 commit, 0 modification sitemap, 0 fichier HTML touché | ⏸ aucune action — attendre prochaine indexation Google (sitemap-delta suivante) |

**Dernière MAJ : 2026-09-18 BST — **round 9+ livré** : Featured Snippet block + FAQPage 16→18 + LocalBusiness knowsAbout 12→14 + Sinais Antecipados section + maillage interne vers `/blog/cano-entupido-gordura-solucoes.html` + dateModified bump 2026-09-18. PR #396 (6 commits cumulés) en attente GO Filipe (R7). Cf. entrée détaillée ci-dessus (t_fc13fae0).**
