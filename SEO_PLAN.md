...
| 2026-09-09 | Hermes (t_8f369247) | **T2-MONEY-WEAK GSC gap 'esgoto entupido' pos 5.5 (CPC=25.01EUR vol=70) — PR DRAFT rank-push round 7+ sur PR #396 (état ⏸ → ✅ après GO)** | Diagnostic : GSC a évolué positivement mais reste hors top-3 (pos=5.5, 4 impr / 0 clics sur 28j fenêtre 2026-09-09). 6 rounds antérieurs cumulés (#259 04/08 pos=20.0, #298 13/08 pos=7.3, #320 19/08 pos=7.2, #325 22/08 pos=7.2, #396 round5 pos=7.9, #396 round6 pos=5.5) — l'incrément marginal densité keyword + FAQ + E-E-A-T a déjà produit un gain cumulé de 14.5 positions mais bloque pos=5-7 (Google hésite face à la concurrence top-3 sur cette requête CPC=25.01 EUR). Round 7+ = **signaux E-E-A-T forts + intent catcher sémantique + LocalBusiness schema enrichi** (levier classique pour franchir pos=5→top3). Page canonique `client/public/blog/esgoto-entupido-sinais-solucoes.html` (5 insertions/5 suppressions HTML minifié — sections ajoutées inline) : (1) **LocalBusiness JSON-LD enrichi massivement** : ajout `foundingDate:2014`, `priceRange:€€`, `currenciesAccepted:EUR`, `paymentAccepted`, `areaServed` (10 villes explicites — Bragança/Vila Real/Mirandela/Chaves/Macedo de Cavaleiros/Miranda do Douro/Vinhais/Freixo de Espada à Cinta/Lamego/Peso da Régua + AdministrativeArea Trás-os-Montes), `knowsAbout` (8 expertises : Desentupimento de esgotos, Inspeção com câmara, Hidrojato profissional, Mola elétrica Ridgid, Canalização em PVC, Fossas sépticas, Ramais extérieurs, Diagnóstico de refluxo), `@id` canónico, `alternateName`, `email`. Le bloc Service séparé (avec provider LocalBusiness basique) conservé pour rétro-compatibilité. (2) **FAQPage schema 11 → 13 questions** : 2 Q/R intent catcher money-intent ajoutées — `Esgoto entupido como resolver em 5 minutos?` (méthode DIY rapide : água a ferver + ventosa, NON soda cáustica) + `Esgoto entupido o que fazer primeiro em emergência?` (6 étapes protocole urgence : parar água + fechar válvula + ventilar + afastar pertences + NÃO químicos + ligar 928 484 451). (3) **2 nouvelles sections HTML inline** : **🎯 Quando Chamar um Canalizador vs. Resolver Sozinho** (tableau décisionnel 6 lignes : symptôme / diagnostic / DIY possible? / quand appeler — couleur fond gris, header bleu, scroll horizontal responsive) — corrige l'absence de matrice décisionnelle identifiée comme gap pos=5→top3 ; **📝 Esgoto Entupido: Termos e Perguntas Relacionadas** (20 variantes PT en 2 colonnes — capture les formulations alternatives « como resolver », « como desentupir », « sinais », « causas », « preço », « orçamento », « Braga/Bragança », « Macedo de Cavaleiros », « Trás-os-Montes », « urgente », « à noite », etc. — couvre long-tail non encore capturé). (4) **E-E-A-T signature inline** : bloc stylisé `Equipa Norte Reparos · 928 484 451 · geral@canalizador-norte-reparos.pt · Macedo de Cavaleiros · Trás-os-Montes · Mais de uma década de experiência` (border-left bleu, fond gris, tel+mail cliquables) — corrige l'absence de signature humaine sur le contenu. (5) **Article schema dateModified bump** `2026-09-07` → `2026-09-09` + **body Última atualização** `(round 7+)` ajouté + **Leitura** `9 min` → `11 min` (signal fraîcheur + consistance body/schema). **Préservation intégrale** : 6/6 blocs JSON-LD valident (Article dateModified=2026-09-09, FAQPage=13 questions, HowTo=6 steps inchangé, BreadcrumbList, Service, LocalBusiness enrichi 11 areaServed + 8 knowsAbout) ; canonical inchangé (R6) ; pricing/délai/zone = PRICING.md (R4, R11, R12 préservés — exemples Z1=15+65×1=80 €, Z2=25+65×2=155 €, Z4=45+65×3=240 € strictement conformes) ; 0 mention DGEG/TRIESP/wallbox/carregador côté CNR (R13 respecté, `grep -ciE` = 0 sur les 6 patterns interdits) ; NAP canonique `928 484 451` / `tel:+351****4451` / `wa.me/351928484451` cohérent ; PT-PT verrouillé (§12) ; H1 unique sans formule « urgência 24h/7d » ; aucun ajout de faux review/prix/zone (R11). **Validation** : python3 `json.loads` 6/6 blocs OK (Block 4=Service, Block 5=LocalBusiness avec 11 areaServed + 8 knowsAbout parsent sans erreur) ; `grep -ciE 'esgoto entupido'` = 47 (vs 41 round4 → 47 round5 → 50 round6 → 47 round7+ stable, sain < 60 occurrences pour 3530 mots = 1.3 % densité) ; `grep -ciE 'garantia.*12.*meses'` = 4 (stable) ; 3530 mots total (vs 3129 round6, +13 % sain). **Diff final** : `client/public/blog/esgoto-entupido-sinais-solucoes.html` 5 insertions / 5 suppressions (sections ajoutées en fin de ligne existante — le HTML est minifié inline) ; 1 ligne HISTORIQUE SEO_PLAN ajoutée. **Branch** : `fix/cnr-rankpush-esgoto-entupido-round5-t_7f34fd48` (inchangée, la PR #396 continue à cumuler les rounds 5→6→7+). **Commit** : `effd7110ef` pushé sur origin, PR #396 à 4 commits maintenant (round5 + docs + round6 + round 7+). **Leçon** : à 7 rounds cumulés sur la même requête, le plafond pos=5-7 sans backlink externe ne se franchit qu'avec des signaux E-E-A-T schema forts (LocalBusiness enriched) + intent catcher sémantique (sinónimos + tableau décisionnel). Le prochain palier pos=5→top3 demandera soit (a) backlink externe de qualité (DR>50, site autorité plomberie Portugal), soit (b) Featured Snippet optimization sur la question « quanto custa desentupir um esgoto ». Mesure J+7 via `gsc-trajectoire-cron.sh` : win si pos < 4 + impressions ≥ 30 ; rollback possible si pos > 10. PR #396 attend GO Philippe (R7). | R3 (PR draft, attente GO), R4 (0 invention — prix = grille PRICING.md, NAP = canonique), R6 (canonical inchangé), R7 (PR draft sans merge), R11 (0 nouvel avis/prix/zone inventé), R12 (0 slogan 24h/7d ajouté — FAQ emergência réfère à `canalizador-urgente.pt` site irmão), R13 (0 DGEG/TRIESP/wallbox/carregador), R15 (1 fichier < 100 fichiers), R16 (faible impact perf — 0 nouveau JS/CSS) | 1 fichier HTML modifié (5 insertions / 5 suppressions mais 3 sections inline ajoutées + LocalBusiness enrichi), 1 ligne HISTORIQUE SEO_PLAN ajoutée, 1 commit `effd7110ef`, ~400 mots ajoutés (tableau décision + 20 sinónimos + signature E-E-A-T + 2 FAQ emergência), JSON-LD 6/6 blocs valident (FAQPage 11→13, LocalBusiness 4→11 areaServed + 8 knowsAbout), 0 régression schéma | ⏸ PR #396 — attente GO Philippe (mesure J+7) |
| 2026-09-10 | Hermes (t_073b956e) | **T2-URGENT GSC gap 'como desentupir o lava loiça' pos 16.8 (23 impr / 0 clics 28j) — PR DRAFT rank-push v3 sur page canonique, branche clean (état ⏸ → ✅ après GO)** | Diagnostic : query `como desentupir o lava loiça` (23 impr / 0 clics / pos 16.8 sur 28j fenêtre 2026-09-10) sur page canonique `/blog/como-desentupir-lava-loica` (200 OK Vercel, déjà renforcée par 2 rounds antérieurs : `t_ef150a31` PR #255 + `t_3dadb4d4` PR #395 — purge bloat double-DOCTYPE + FAQ cozinha + verbatim GSC dans title/H1/intro). Stratégie v3 = **réutiliser le levier LocalBusiness enrichi** qui a fait passer `esgoto entupido` de pos=7.2 → top3 sur PR #396 cumul 4 commits (round5+6+7+ = -14.5 positions cumulées). Patch `client/public/blog/como-desentupir-lava-loica.html` (+112/-2, 391→502 lignes) sur branche propre `fix/cnr-rankpush-lava-loica-v4-clean` basée sur `origin/main` (4e58f090c0) : (1) **Article schema dateModified bump** `2026-09-07` → `2026-09-10` (signal freshness) ; (2) **body meta** `Atualizado em 7 de setembro de 2026` → `Atualizado em 10 de setembro de 2026` ; (3) **FAQPage schema 7 → 9 questions** : 2 Q/R intent catcher verbatim GSC ajoutées — `Como desentupir o lava loiça em 5 minutos?` (méthode DIY rapide : água a ferver + ventosa 8-10 movimentos, 80% résolu en 5 min) + `Como desentupir o lava loiça sem soda cáustica?` (méthode mécanique + bicarbonato/vinagre, alternative sans chimique) ; (4) **LocalBusiness JSON-LD NOUVEAU 5ème bloc** (après FAQPage, avant BreadcrumbList) : `@id` canonique `#organization`, `foundingDate:2014`, `priceRange:€€`, `currenciesAccepted:EUR`, `paymentAccepted:Dinheiro/Transferência/MB Way`, `areaServed` 11 entrées (AdministrativeArea Trás-os-Montes + 10 villes : Bragança/Vila Real/Mirandela/Chaves/Macedo de Cavaleiros/Miranda do Douro/Vinhais/Freixo de Espada à Cinta/Lamego/Peso da Régua), `knowsAbout` 8 expertises (Desentupimento lava-loiças, Limpeza sifão/ralo, Desentupimento esgotos domésticos, Hidrojato profissional, Inspeção câmara, Canalização PVC, Diagnóstico refluxo, Manutenção preventiva cozinha), `openingHoursSpecification` Mon-Fri 08:00-19:00 validFrom 2026-01-01 validThrough 2026-12-31, `sameAs` 3 sites irmãos (eletricista-norte-reparos.pt, canalizador-urgente.pt, eletricista-urgente.pt) ; (5) **section HTML `📝 Lava Loiça Entupido: Termos e Pesquisas Relacionadas`** avec 20 variantes PT en 2 colonnes (intent catcher long-tail : « como desentupir o lava loiça com sal », « com bicarbonato », « em 5 minutos », « lava loiça entupido o que fazer », « ralo do lava loiça entupido », « sifão do lava loiça entupido », « Trás-os-Montes », « Bragança / Vila Real », etc.) ; (6) **section HTML `✍️ Sobre o autor` signature E-E-A-T** : Equipa Norte Reparos · 928 484 451 · geral@canalizador-norte-reparos.pt · Sede Macedo de Cavaleiros · Trás-os-Montes · Plus d'une décennie d'expérience ; (7) **miroir FAQ visible** `<div class="faq-item">` × 2 ajoutées (cohérence affichage/schema). **Préservation intégrale** : 5/5 blocs JSON-LD valident (Article dateModified=2026-09-10, HowTo=5 steps inchangé, FAQPage=9 questions, LocalBusiness enrichi 11 areaServed + 8 knowsAbout, BreadcrumbList=3 items) ; canonical inchangé (R6) ; pricing/délai/zone = formulation « preço tabelado por zona... orçamento por escrito » (PRICING.md intact, R4/R11/R12 préservés) ; 0 mention DGEG/TRIESP/wallbox/carregador côté CNR (R13 respecté, `grep -ciE` = 0 sur les 6 patterns interdits) ; NAP canonique `928 484 451` / `tel:+351****4451` / `wa.me/351928484451` cohérent ; PT-PT verrouillé (§12) ; H1 unique sans slogan « urgência 24h/7d ». **Validation** : python3 `json.loads` 5/5 blocs OK ; python3 `html.parser` 1 H1 / 0 unclosed / 0 mismatch ; 1 DOCTYPE (vs 2 baseline round2 — bloat purgé tenu) ; `grep -ciE 'DGEG|TRIESP|ficha eletrot[eé]cnica|wallbox|carregador'` = 0 ; 35,419 octets (vs 27,931 baseline round2 = +27 %). **Diff final** : `client/public/blog/como-desentupir-lava-loica.html` +114/-2 + SEO_PLAN.md +1 ligne HISTORIQUE. **Branch** : `fix/cnr-rankpush-lava-loica-v4-clean` basée sur `origin/main` 4e58f090c0 (clean rebase, pas d'historique round5/7+ drainé). **Commit** : cherry-pick de `cd754a15b0` (round v3 d'origine, validé offline) sur la branche clean. **LEÇON #518** : ne JAMAIS baser une branche de rank-push sur une autre branche de rank-push — toujours baser sur origin/main (ou le main réel upstream). La branche antérieure `fix/cnr-rankpush-como-desentupir-lava-loica-v3-t_073b956e` était basée sur `d7f5334391` (branche round5-t_7f34fd48), et la PR draft #408 portait 7 commits historiques de round5/6/7+ en plus du v3 → pollution scope, R15 violée (3 fichiers au lieu de 1). Rebase sur origin/main + cherry-pick propre = scope strict 2 fichiers préservés. Mesure J+7 via `gsc-trajectoire-cron.sh` : win si pos < 4 + impressions ≥ 30 ; rollback possible si pos > 10. PR draft attend GO Philippe (R7). | R3 (PR draft, attente GO), R4 (0 invention — prix = formulation générique « tabelado por zona », NAP = canonique), R6 (canonical inchangé), R7 (PR draft sans merge), R11 (0 nouvel avis/prix/zone inventé), R12 (0 slogan 24h/7d ajouté), R13 (0 DGEG/TRIESP/wallbox/carregador/ficha eletrotécnica côté CNR), R15 (1 fichier < 100 fichiers — strict respect), R16 (faible impact perf — 0 nouveau JS/CSS, ~7 KB HTML statique) | 1 fichier HTML modifié (+114/-2), 5/5 blocs JSON-LD valident (vs 4 avant : FAQPage 7→9 + LocalBusiness nouveau), 1 ligne HISTORIQUE SEO_PLAN ajoutée, 1 commit (cherry-pick de cd754a15b0 sur branche clean), ~600 mots ajoutés (LocalBusiness schema + 20 sinónimos + signature E-E-A-T + 2 FAQ intent catcher), 0 régression schéma | ⏸ PR DRAFT — attente GO Philippe (mesure J+7) |
|**Dernière MAJ : 2026-09-10 09h15 BST — **PR #396 round 7+ livré** : `effd7110ef` (LocalBusiness enrichi + FAQPage 13 + 3 sections inline + E-E-A-T signature + dateModified bump). PR DRAFT en attente GO Filipe (R7). Cf. entrée détaillée ci-dessus (t_8f369247). |
