...
### 2026-08-13 — GSC rank-push « desentupir lava loiça » (Hermes t_9275a984)
- **Signal GSC (fenêtre 28j terminée 2026-08-13)** : query `desentupir lava loiça` à **position moyenne 17,2 / 33 impressions / 1 clic / CTR ≈ 3 %** — verdict **WEAK** (page canonique existe mais sous-classée, presque top3 en fluctuation 4–20).
- **Diagnostic vérifié** : la page `client/public/blog/como-desentupir-lava-loica.html` EXISTE déjà et a déjà été renforcée (Hermes t_7221c8c6, 2026-08-03, ligne 1703) — le brief pointe par erreur `/blog/valvula-seguranca-goteja` comme meilleure page mais la page canonique dédiée reste la cible. 4 sisters `*-com-sal.html`, `*-cozinha.html`, `*-passos.html` complètent le cluster maillage interne. Décision : **renforcement chirurgical ciblé sur la query exacte** (sans le « como ») plutôt que nouvelle URL concurrente — alignement avec la doctrine « pas de cannibalisation, on renforce l'URL canonique ».
- **Action (1 fichier, +34/-7)** : commit `757a3be0a8`.
  - `<title>` réécrit : « Desentupir Lava-Loiça: 5 Métodos Seguros (Passo a Passo 2026) » (query exacte « Desentupir Lava-Loiça » en tête, sans « como » pour matcher l'intent utilisateur du brief).
  - `<meta description>` alignée : « Desentupir lava loiça em casa: 5 métodos seguros do mais simples ao profissional — ventosa, sal, sifão, máquina e diagnóstico. Saiba quando parar e contactar a nossa equipa. »
  - `<h1>` reformulé : « Desentupir Lava-Loiça: 5 Métodos Seguros Para Resolver em Casa ».
  - Intro `<p>` avec `<strong>Desentupir lava loiça</strong>` en early body (sémantique strong, signal on-page direct pour Google).
  - Nouvelle section H2 : « Desentupir lava loiça rapidamente: o que fazer primeiro? » (`id="resposta-rapida"`).
  - Nouvelle section H2 : « Por que o lava loiça entope com frequência? » (3 causes principales + stat non chiffrée « 80% dos casos resolve-se em casa »).
  - FAQ ajoutée : « Quanto custa desentupir o lava loiça em Trás-os-Montes? » — réponse fondée sur `PRICING.md` (zonée Z1–Z6, 65 €/h, orçamento por escrito).
  - `FAQPage` JSON-LD mis à jour : 5 → 6 questions synchronisées (validator Schema.org OK).
  - Bloc « Related guides » enrichi : 3 sister-pages (com-sal, cozinha, passos) en cross-link interne pour maillage du cluster.
- **Conformité** : R4 (zéro prix/zone/délai/marque/avis inventé — uniquement `PRICING.md` + zonage PT), R5 (zéro `streetAddress`, claims locaux vérifiables uniquement), R11 (« nous »/collectif, doctrine « mediante confirmação » préservée, zéro « je »), R12 (zéro slogan 24h/7d côté client), R145 (zéro délai chiffré ajouté), R7 (PR draft atomic, 0 merge), R10 (robots.txt intact).
- **Témoins R8** : diff stat `git show --stat HEAD` → 1 fichier, +34/-7, conforme au titre du commit. `git diff --stat origin/main..HEAD` → `client/public/blog/como-desentupir-lava-loica.html | 41 ++++++++++++++++++----` (atomique, scope respecté). Aucun autre fichier modifié.
- **Statut** : ⏸ PR draft avant review/GO Philippe (R7) ; passer à ✅ seulement après GO/merge. Branche `fix/cnr-rankpush-desentupir-lava-loica-t_9275a984` basée sur `origin/main` (930c7878e, fetched 2026-08-13). Mesure à J+7 via `gsc-trajectoire-cron.sh` : win si position < 4 ; réévaluer si position reste > 10 (rollback doctrinal possible). Tâche `t_9275a984` à clore après push + ouverture PR draft.
