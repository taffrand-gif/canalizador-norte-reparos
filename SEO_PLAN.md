...
| X-MAIL | Email `privaterelay.appleid.com` publié comme contact — 7 fichiers | HAUTE | FAIT | #380 CNR · #437 ENR · #317 CU · #366 EU (tous MERGED 14:31 02/09) | — | `privaterelay\.appleid\.com` · CNR 3 · ENR 2 · CU 1 · EU 1. Correctif : `contacto@canalizador-norte-reparos.pt` (plomberie), `geral@eletricista-norte-reparos.pt` (électricité). ⚠️ **Note post-merge (L#035)** : CU/EU patch no-op — `public/.well-known/ai-plugin.json` répond 404 en prod (sites urgence = statique, racine servie = racine du repo, pas `client/public/` comme CNR/ENR). Sites urgence : `.well-known/` à la racine jamais servi. Découverte d'un signal GEO absent sur CU/EU. À ouvrir comme chantier `.well-known-CU-EU`. |
| X-ORC | « orçamento gratuito / grátis » | HAUTE | EN_COURS | — (PR #363 fermée/remplacée 31/08 21h31 par Philippe, voir leçon L#020) | 🛑 **PR #363 fermée 31/08 21h31 — remplacée.** Motif Philippe : branche `fix/cnr-x-orc-v1-2026-08-30` divergeait de main de 181 fichiers (+2399/-4170, 10 commits behind), rebase demandait de rejouer le patch sur 10 merges main parallèles avec risque d'annuler du travail mergé. Le prédicat originel `[Oo]r[çc]amento[^<>]{0,40}` traversait les phrases (corrigé sur ENR par #370). **Reprise à zéro** avec prédicat corrigé + substitution locution-à-locution sur base `992198dfb`. · **Mesure fresh @ `3e287ea6aa` (post-#371)** avec prédicat corrigé `[^<>.!?]` : **CNR 71 · ENR 4 · CU 0 · EU 0 = 75 fichiers** (vs 96 ancien prédicat). · **17 résiduelles URLs `*-orcamento-gratis.html`** documentées par Philippe dans le PR body #363 (hors-périmètre renommage massif). · Reprise = nouvelle PR dédiée, hors scope de cette tâche — worker dédié requis avec re-mesure + substitution locution-à-locution (cf. R-canon 11/08) : chiffrage à distance → `estimativa sem custo` ; autre contexte → `orçamento por escrito antes da intervenção`. GO Philippe requis. | `[Oo]r[çc]amento[^<>.!?]{0,40}(gratuit\|gr[áa]tis)` · fenêtre 40 c, sans traverser de balise ni fin de phrase. **Mesuré @ `3e287ea6aa` (prédicat corrigé post-#370, hors URLs `*-orcamento-gratis.html` et hors JSON-LD `name`) : CNR 71 · ENR 4 · CU 0 · EU 0 = 75 fichiers.** ⛔ EXCLUSION : `(diagn[óo]stico\|an[áa]lise\|avalia[çc][ãa]o\|inspe[çc][ãa]o)[^<>.!?]{0,30}gratuito` — non violantes (R12). |
...
| X-GAR | Garantie → 12 meses — **RÉGRESSION X-GAR-6M** | HAUTE | EN_COURS | #382 (CNR), #449 (ENR) | — | L#021 chantier « clos » exige mesure EXHAUSTIVE : la passe `6 meses → 12 meses` du 30/08 a raté la formulation `Garantia de 6 meses` (CF files signals). L#022 lesson : périmètre gate = arbre production COMPLET. L#035 lesson : curl prod-live (cf. PR #382 body). Reprise : CAT-A `Garantia de 6 meses` + `garantia de 6 meses` + `Garantia 6 meses` + cellulaire tableau `>6 meses</td>` + footer ` / 6 meses`. Hors-périmètre : `6 meses` seul (178+ occ CNR, contextes mixtes — triage manuel). Idiomatique `…que falham em 6 meses` (TransparencePrix.tsx ENR, critique low-cost) **NON** patché — préserver R11 prose. |
