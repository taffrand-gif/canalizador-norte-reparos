...
### ⚠️ BLOCAGE DOCTRINAL 2026-08-04 (t_a5287105) — rank-push query 'canalizador urgente' (CPC=14.63€) sur CNR = CONFLIT R12 direct, re-routage vers CU

> **Périmètre strict** : la query GSC money **'canalizador urgente'** (DataForSEO vol=170/mois PT-2620, **CPC=14.63€ = le plus cher du marché plomberie PT**, score 2487.10) avec GAP total en GSC 28j (1 impr / 0 clic / pos=30) sur **CNR (canalizador-norte-reparos.pt)**.

**Diagnostic AVANT patch — conflit doctrinal majeur détecté** :

Le brief kanban t_a5287105 demande un rank-push "canalizador urgente" sur **CNR**. Or la doctrine verrouillée du projet **interdit explicitement** cette cible :

|| Source | Verdict |
||---|---|
|| `SEO_PLAN.md` ligne 23 (audit 2026-07-01, item P6) | "Mots-clés (P6) : `shared/seoKeywords.ts` **contamine l'intent** — cible `canalizador urgente`/`24h`/`resposta prioritária` sur un site installation → **cannibalise le domaine urgente**" |
|| `SEO_PLAN.md` ligne 254 (roadmap M2) | "[ ] **M2** — Fix `shared/seoKeywords.ts` : retirer `urgente`/`24h`/`resposta prioritária` (**stop cannibalisation domaine urgente**)" |
|| `SEO_PLAN.md` ligne 326 (VISION) | "**Périmètre site : Installation, projets, devis. PAS d'urgence (c'est `canalizador-urgente.pt` qui gère ça)**" |
|| `SEO_PLAN.md` ligne 728 (historique 2026-07-30) | "M2 — purge claims urgence/24h-7d dans `FAQLocal.tsx`... **cannibalisation de l'intent `canalizador-urgente.pt`**" |
|| `SEO_PLAN.md` ligne 944 (doctrine cross-sites) | "❌ Ne PAS ajouter de 'urgente' sur ce site (c'est le job de `canalizador-urgente.pt`)" |
|| `SEO_PLAN.md` ligne 1175 (historique 2026-07-01 21h15) | "M2-exec prototype `canalizador-braganca.html` (CNR) — **angles distincts vs CU urgente** : H1 installation/remodelação/certificação, devis 48h, garantia 1 an, horário comercial **seg-sex**" |
|| `AGENTS.md` §13 (verrouillée 30/07/2026) | Pattern : chaque site a son scope strict (chargeur VE = EU/ENR only, JAMAIS CNR/CU) |
|| Repo frère | `~/work/Sites/canalizador-urgente/` (CU) **EXISTE et fonctionne** (cf. SEO_PLAN ligne 888, 924) — c'est LUI qui doit ranker "canalizador urgente" |

**Inventaire live 04/08 — état CNR vs la query 'canalizador urgente'** :

|| Dimension | État 04/08 (live) | Note |
||---|---|---|
|| Pages PAA `canalizador-urgente-<localidade>.html` | **77 fichiers** | Pattern PAA, noindex,follow — déjà en place |
|| Pages blog `canalizador-urgencia-<localidade>-guia.html` | **18 fichiers** | Guides détaillés par localité |
|| Pages `urgencia-canalizador-<localidade>.html` | **40+ fichiers** | Pages urgence par localité |
|| Pages TSX `client/src/pages/urgencias/*Urgente*.tsx` | **16 fichiers** | Dont `UrgentePorto`, `UrgenteVilaReal`, etc. |
|| Pages TSX orphelines (pas de route App.tsx) | **2 fichiers** | `Urgencia.tsx` (441 L), `UrgenciaFugaAgua.tsx` (320 L) |
|| Page canonique statique `/urgencias-24h` | `client/public/urgencias-24h.html` (62 L, ~11.8 KB) | Existe, mais `<title>` = "💧 Canalizador em Trás-os-Montes" (NE cible PAS la query exacte) |
|| H1 de `/urgencias-24h` | "🚨 Canalizador Urgente — Atendimento 24h" | Match exact query ✅ — mais titre incohérent |
|| Schéma `Plumber` JSON-LD | name="Canalizador Urgente Trás-os-Montes" + openingHours 00:00-23:59 + sameAs canalizador-urgente.pt | Cohérent avec la doctrine cross-sites ✅ |

**Constat** : la query "canalizador urgente" est **physiquement couverte** par 100+ pages sur CNR (PAA + guides + urgence-localidade). Le GAP GSC (0 impr 28j) ne vient pas d'une absence de pages — il vient du **mismatch d'autorité de domaine** : Google sait que `canalizador-urgente.pt` est le domaine "urgente" et `canalizador-norte-reparos.pt` est le domaine "installation". Renforcer la page `/urgencias-24h` de CNR pour la query "canalizador urgente" :

1. **Viole R12** directement (cannibalisation `canalizador-urgente.pt`) — c'est exactement le chantier que M2 demande de CORRIGER en sens inverse.
2. **Invalide 6 mois de purge R12** (cf. SEO_PLAN lignes 1175, 728, 774, 1855) — pattern "purge puis re-création" = boucle vaine.
3. **Recycle le budget crawl** de CNR sur la mauvaise intent (Google va continuer à classer `canalizador-urgente.pt` au-dessus pour cette query).
4. **N'ouvre pas de valeur** : les 100+ pages PAA/urgence-localidade de CNR sont déjà optimisées pour des variantes longue traîne ("canalizador urgente bragança", "canalizador urgente mirandela") qui ramènent du trafic qualifié via SEO local. La query HEAD "canalizador urgente" n'est PAS dans le périmètre business de CNR.

**Décision appliquée (mode autonome, R3 + R7)** :

- **0 PR draft ouvert** sur cette tâche. Le brief entre en conflit direct avec la doctrine verrouillée du projet (CNR ≠ domaine urgente). Tenter une PR = violer R12 + défaire le chantier M2.
- **Re-routage提议** : la query money "canalizador urgente" doit être travaillée sur le repo **CU = `~/work/Sites/canalizador-urgente/`** (`canalizador-urgente.pt`), pas CNR. Le pattern est identique à t_e618703b : rank-push sur le bon repo, scope strict.
- **Action concrète CNR** (alternative R12-conforme si Philippe veut quand même renforcer CNR sur cette query malgré tout) : créer une tâche kanban `t_reroute-cu-canalizador-urgente` (assignee=default, parents=[t_a5287105]) qui vise `/urgencias-24h` de **CU**, pas CNR. Le brief est re-routé vers le bon repo.
- **Tâche kanban t_a5287105 BLOQUÉE** (kind=needs_input) — décision Philippe requise : (a) re-router vers CU ; (b) confirmer exception R12 sur CNR avec angle "installation réactive ≠ urgence 24h" (peut-être reformuler la page `/urgencias-24h` pour qu'elle cible "resposta rápida instalação" plutôt que "urgente" canonique) ; (c) classer NO-OP (la query money est hors-périmètre business CNR, le ROI SEO est sur CU).

**Sources primaires vérifiées 04/08** :
- Repo CU existe : `ls ~/work/Sites/canalizador-urgente/` = AGENTS.md + CLAUDE.md + SEO_PLAN.md + PRICING.md + ~700 fichiers.
- `git log` branche courante `feat/cnr-rankpush-desentupir-canos-v2-t_06389a27` = ancêtre de main (ee0d51c0b), 0 commit lié à la query "canalizador urgente".
- `git status` working tree = 940 fichiers modifiés (dont 18 `canalizador-urgencia-*-guia.html` issus de la tâche t_06389a27 précédente, 0 lié à cette tâche).

**Leçons codées (LECONS.md)** : #CNR-BRIEF-DOCTRINAL-MISMATCH-01 (brief kanban doit pré-filtrer les conflits de scope site vs doctrine avant dispatch — éviterait ce blocage), #CNR-CANNIBALISATION-INTENT-01 (renforcer "urgente" sur CNR = violer R12 verrouillée 30/07/2026, le rank-push money sur la query HEAD "canalizador urgente" appartient à `canalizador-urgente.pt`).
...
|| 2026-08-04 | Hermes (Kanban t_89000c9c) | **Rank-push GAP `esgoto entupido`** | Audit de la couverture : URL existante `/blog/esgoto-entupido-sinais-solucoes.html` (pas de duplication). Renforcement chirurgical du title, meta description, keywords et H1 pour alignement sémantique exact, sans prix localité inventé. | R3/R7 : travail préparatoire uniquement, branche déjà chargée de modifications préexistantes ; R4/R11 : contenu factuel existant conservé ; R12 : CNR reste hors périmètre urgence ; PR/merge séparés après review. | 1 fichier HTML modifié + audit source `PRICING.md` rechargée (65€/h, Z1=15€…Z6=65€) ; aucune mention DGEG/TRIESP ajoutée. FAQ existante contient des claims incohérents hérités et nécessite une passe séparée de conformité avant publication. | 🛑 STOP — review Filipe requise avant PR draft/merge |
...
- **État** : ⏸️ PR draft sur `feat/cnr-rankpush-desentupir-canos-v2-t_06389a27` (branche renommée après collision de nom avec un autre worker `t_e618703b` qui occupait la branche initiale `feat/cnr-rankpush-desentupir-canos-t_06389a27` avec un sujet différent — query `desentupimento` ≠ `desentupir canos`), commit `158557163` poussé OK, aucun merge (R7) ; attente GO Filipe. Mesure d'impact J+7 via `gsc-trajectoire-cron.sh` (recette : 1ères impressions > 0 en 28j = gain indexation ; pos < 10 = gain ranking).
| **Note collision** : la branche homonyme `feat/cnr-rankpush-desentupir-canos-t_06389a27` est désormais occupée par le worker `t_e618703b` (PR #260 DRAFT, SHA distant `4f3e99e9`, query `desentupimento` sur `/desentupimentos`, sujet distinct — voir `t_e618703b` worker_context). Le présent worker a renommé sa branche en `…-v2-t_06389a27` pour éviter tout force-push destructeur. **À fusionner : les 2 PRs portent sur des fichiers différents** (PR #260 = `client/src/pages/Desentupimentos.tsx` + `/desentupimentos` HTML, présente PR = `client/src/pages/blog/GuiaDesentupirCanos.tsx` + `/blog/guia-desentupir-canos.html`). Aucun conflit technique attendu entre les 2 PRs si elles sont mergées séparément.

### 2026-08-04 — Re-validation t_8da1e3d4 (leçon #469 anti-doublon appliquée)
- **Contexte** : pool-keeper a re-poussé le chantier `desentupir canos` (T1-MONEY CPC=6.51€ score=8463) sous un nouvel id `t_8da1e3d4` après que `t_06389a27` ait livré PR #261 DRAFT le 04/08 09:53 (commit `158557163`).
- **Diagnostic re-validation live** : PR #261 toujours DRAFT sur `feat/cnr-rankpush-desentupir-canos-v2-t_06389a27` (synchro origin OK, 0 commit divergent). Re-mesure post-livraison :
  - titre HTML = `Desentupir Canos: Causas, Métodos Caseiros e Quando Chamar um Canalizador | Norte Reparos` ✓
  - H1 HTML = `Desentupir Canos: Causas, Métodos Caseiros e Quando Chamar um Canalizador` ✓ (match keyword exact + intent « comment faire »)
  - densité keyword `desentupir canos` : TSX = 16 (cf. worker_context), HTML = 18 (était 0 avant patch) — sain, pas de stuffing
  - FAQPage JSON-LD : 8 questions (était 7 avant patch), 1ère = « Como desentupir canos em casa sem partir a tubagem? » ✓
  - interdits R5/R11 : 0 occurrence DGEG/TRIESP/wallbox/carregador VE dans le fichier patché ✓ (purge 03/08 respectée, scope plomberie strict)
  - tarifs : 100% PRICING.md, 0 fourchette inventée ✓
  - canon self conservé, dateModified 2026-07-17 → 2026-08-04 (signal fraîcheur) ✓
- **Décision** : **NO-OP légitime** sur le livrable. Leçon #469 (anti-doublon cache-lag) s'applique — le chantier est clos depuis 10:49 BST, ré-ouvrir une PR identique violerait la doctrine et générerait une cascade de conflits sur `GuiaDesentupirCanos.tsx` (cf. leçon sœur #CNR-AUDIT-PR-451).
- **Action réalisée sur ce run** : (1) re-validation live des 5 invariants (title/H1/densité/FAQ/interdits) ; (2) consignation traçable dans ce `SEO_PLAN.md` ; (3) marquage `kanban_complete` sans re-push Git (préserve PR #261 et la branche intactes, pool-keeper ne re-pousse pas une 3e fois).
- **Gates R7** : PR #261 NON MERGÉE — attente GO explicite Filipe (R7 « pas de merge sans validation »). Mesure d'impact à J+7 via `gsc-trajectoire-cron.sh` : recette de succès = 1ères impressions > 0 sur `desentupir canos` en 28j (gain indexation) OU pos < 10 (gain ranking). Si la recette échoue à J+7, déclencher une vague R15 (renforcement complémentaire) plutôt qu'une 4e itération du même livrable.
- **Refs** : `t_06389a27` (livraison initiale, commit `158557163`, PR #261), `t_e618703b` (chantier voisin sur `desentupimento`, PR #260 — collision de branche documentée, 0 conflit technique attendu), leçon #469 (anti-doublon cache-lag), leçon #CNR-RANKPUSH-DESENTUPIMENTO-01 (renforcement > duplication même intent singulier/pluriel PT).

### 2026-08-04 — NO-OP légitime t_1af269de (leçon #469 anti-doublon, 3e ré-injection 'desentupimento')
- **Contexte** : pool-keeper a re-dispatché le même chantier `desentupimento` (T1-MONEY CPC=12.76€ vol=1600 score=20416, GAP GSC pos=None 28j) sous id `t_1af269de` après que `t_e618703b` ait livré PR #260 DRAFT le 04/08 09:51 (commit `4f3e99e9e`) et que `t_b61b8f04` ait déjà consigné un NO-OP légitime par leçon #469 (commit `05d10ff60` 11:16). 3e injection consécutive du même chantier (même score, même query, même KPI recette, même cible canonique `/desentupimentos`).
- **Diagnostic re-validation live 04/08 13:23 BST** :
  - Branche courante `feat/cnr-rankpush-canalizador-urgente-t_f3259fea` (HEAD = `ee0d51c0b`) **contient déjà** le commit `4f3e99e9e` (PR #260) — vérifié `git merge-base --is-ancestor 4f3e99e9e HEAD` = TRUE, `diff <(git show 4f3e99e9e:Desentupimentos.tsx) <(git show HEAD:Desentupimentos.tsx)` = vide.
  - PR #260 (`fix(cnr,seo): rank-push query 'desentupimento' sur /desentupimentos`) **état GitHub** : `state=OPEN`, `mergeStateStatus="DIRTY"`, `mergeable="CONFLICTING"` (⚠️ conflit de merge avec main post-PR #263 hero homepage + autres merges récents 04/08), Vercel preview SUCCESS.
  - Fichier `client/src/pages/Desentupimentos.tsx` actuel sur HEAD = `Title = "Desentupimento em Trás-os-Montes | Orçamento por Escrito | Desde 65€/h"`, H1 = `"Desentupimento em Trás-os-Montes"`, schema Service `areaServed: AdministrativeArea "Trás-os-Montes"`, 6 questions FAQPage, UnitPriceSpecification 65€/h, 12 liens localité (Macedo/Mirandela/Bragança/Chaves/Valpaços/Mogadouro/Vinhais/Miranda do Douro/Torre de Moncorvo/Freixo de Espada à Cinta/Lamego/Vila Real).
  - Conformité R4/R5/R11/R12/R145 OK : 0 DGEG/TRIESP/wallbox (purge 03/08), 0 fourchette inventée, 0 délai chiffré, 0 marque/modèle spécifique (Ridgid K9-102 retiré), géo-neutre (0 geoMidpoint Macedo), CTA `Pedir Orçamento` partout (R12).
  - Tarifs : 100% PRICING.md (65 €/h + Z1=15 → Z6=65 + majoration +50 %), canon self conservé.
- **Décision** : **NO-OP légitime** sur le livrable SEO. Leçon #469 (anti-doublon cache-lag) doublément appliquée — `t_b61b8f04` (11:16) puis `t_1af269de` (13:23) sur le même chantier, même score, même query. Réouvrir une 3e PR identique violerait la doctrine, ferait doublonner le boulot déjà commité `4f3e99e9e`, et cascaderait des conflits sur `client/src/pages/Desentupimentos.tsx`.
- **🐛 BUG LATENT CONFIRMÉ en prod (escalade R3+R7+R1 maintenue)** : `curl -sIL https://canalizador-norte-reparos.pt/desentupimentos` répond **301 → `/desentupimento` → 404** « Página não encontrada » (vérifié 13:23 BST). Cause historique = règle `vercel.json` ligne 1755 `{"source":"/desentupimentos","destination":"/desentupimento","statusCode":301}` sans destination réelle (le fichier `client/public/desentupimento.html` au singulier n'existe pas, pas plus que la route React `/desentupimento`). Impact direct : PR #260 même mergée = invisible Googlebot tant que le 301 cassé n'est pas corrigé. Travail SEO waste tant que ce bug d'infra persiste.
- **⚠️ Escalade nouvelle (post 11:16 t_b61b8f04) : PR #260 est devenue `mergeable: CONFLICTING`** entre 11:16 et 13:23 (PR #263 hero homepage + autres merges intervenus sur main entre-temps). Philippe doit (a) trancher l'option de fix `vercel.json` (A=self-loop suppression 301 OU B=créer `desentupimento.html` singulier), puis (b) rebase `feat/cnr-rankpush-desentupir-canos-t_06389a27` sur `origin/main` pour résoudre les conflits, puis (c) merger. **STOP validation** sur ces 3 étapes (R1 OpenClaw infra + R7).
- **Action réalisée sur ce run** : (1) audit anti-doublon 3 couches (`t_e618703b` PR #260 commit `4f3e99e9e` + `t_b61b8f04` NO-OP doc `05d10ff60` + `t_1af269de` ce run) ; (2) re-validation live des 8 invariants SEO (title/H1/schema FAQ/schema Service/grille PRICING/12 liens localité/0 interdits R5+R11+R12) ; (3) `curl -sIL /desentupimentos` confirme 301→404 toujours actif en prod ; (4) consignation traçable dans ce `SEO_PLAN.md` ; (5) `kanban_block(reason="review-required: PR #260 + vercel.json 301 cassé")` pour escalader le triple blocage (PR #260 CONFLICTING + bug vercel.json 301 cassé + besoin GO Philippe explicite).
- **Gates R7** : PR #260 NON MERGÉE — attente GO explicite Filipe (R7 « pas de merge sans validation »). Mesure d'impact à J+7 via `gsc-trajectoire-cron.sh` : recette de succès = pos > 0 sur `desentupimento` en 28j (gain indexation/coverture sémantique) ET pages indexées crawlées OK (nécessite fix `vercel.json` d'abord). Si la recette échoue à J+7 même après fix `vercel.json`, déclencher une vague R15 (renforcement complémentaire sur FAQ/H2/H3) plutôt qu'une 4e itération du même livrable.
- **Refs** : `t_e618703b` (livraison initiale sur le même chantier, commit `4f3e99e9e`, PR #260 OPEN DIRTY/CONFLICTING), `t_b61b8f04` (re-validation sœur, NO-OP légitime leçon #469, commit `05d10ff60`), `t_8da1e3d4` (re-validation sœur `desentupir canos`, même pattern #469), leçon #469 (anti-doublon cache-lag), leçon #CNR-RANKPUSH-DESENTUPIMENTO-01 (renforcement > duplication même intent singulier/pluriel PT), leçon #CNR-RANKPUSH-DESENTUPIMENTO-02 (page existante + violations doctrine = cible 1 PR dense, scope strict), `vercel.json` ligne 1755 (301 cassé historique, escalade R1 OpenClaw requise), PR #263 (hero homepage R12/R145 cause du merge CONFLICTING sur #260).

### 2026-08-04 — NO-OP légitime t_f3259fea (leçon #469 anti-doublon étendue R12)
- **Contexte** : pool-keeper a dispatché `t_f3259fea` (DFSEO vol=170 CPC=14.63€ score=2487.10, GAP total 1 impr/0 clic 28j pos 30) avec brief « créer page pilier /canalizador-urgente sur CNR ». Le run 1 (timed_out après 12 min) a déjà créé `client/src/pages/CanalizadorUrgente.tsx` + `client/public/canalizador-urgente.html` (untracked), ajouté la route dans `App.tsx` L235, et ajouté l'URL dans `client/public/sitemap.xml` (gitignoré, dérivé) — **violation R12 caractérisée** (cannibalisation directe `canalizador-urgente.pt`).
- **Doctrine applicable** : (1) `SEO_PLAN.md` §944 — « ❌ Ne PAS ajouter de 'urgente' sur ce site (c'est le job de `canalizador-urgente.pt`) » ; (2) M2 (2026-07-30) — purge claims `urgente/24h/7d` sur CNR précisément pour casser la cannibalisation ; (3) `t_a5287105` (cf. §247) — re-routage explicite de la même query vers CU déjà documenté ; (4) leçon #CNR-CANNIBALISATION-INTENT-01 (codée 04/08).
- **Décision** : **NO-OP légitime**. La query HEAD « canalizador urgente » appartient au périmètre de `canalizador-urgente.pt` (CU), pas CNR (cf. §944 + M2 historisé). Tenter le rank-push sur CNR reproduit exactement la violation que M2 a corrigée en sens inverse il y a 5 jours.
- **Action réalisée sur les runs** : (1) audit anti-doublon ré-effectué (`App.tsx` route L235 ajoutée par run 1, fichier pilier untracked, sitemap.xml gitignoré de toute façon non commité) ; (2) **ROLLBACK complet** : `git checkout -- client/src/App.tsx` (L117 + L234-235 supprimées), `rm client/src/pages/CanalizadorUrgente.tsx client/public/canalizador-urgente.html`, retrait de l'URL pilier du `client/public/sitemap.xml` (gitignoré, régénéré au build depuis `public/sitemap.xml`) ; (3) revalidation finale du run 3 : aucun `CanalizadorUrgente.tsx`, aucun `canalizador-urgente.html`, aucun diff `App.tsx`/sitemap lié à cette tâche, aucun commit sur la branche et `gh pr list --head feat/cnr-rankpush-canalizador-urgente-t_f3259fea --state all` = `[]` ; (4) tâche close en NO-OP terminal, sans override de la doctrine R12.
- **Leçon codée** : #CNR-RE-ROUTAGE-CROSS-SITE-01 — quand le brief assigne un chantier à un repo mais que la query/URL cible est **physiquement hors périmètre** (intent différent = site sœur), la bonne réponse n'est pas « exécuter quand même » mais (a) rollback immédiat, (b) consignation NO-OP, (c) re-routage du chantier vers le bon repo par le pool/orchestrateur. Pattern à appliquer systématiquement pour t_a5287105, et pour tout futur t_* money GAP qui tomberait sur un keyword « urgente » ou « 24 horas » côté CNR.
- **État** : ✅ NO-OP terminal, branche `feat/cnr-rankpush-canalizador-urgente-t_f3259fea` sans commit propre à cette tâche, PR draft JAMAIS ouverte (R7 respectée). Le signal money doit être traité exclusivement sur CU (`canalizador-urgente.pt`) par un chantier dédié, pas par un renforcement CNR.

### 2026-08-04 — NO-OP légitime t_8e6591ba (leçon #469 anti-doublon, 3e réinjection pool-keeper)
- **Contexte** : pool-keeper a re-dispatché une 3e fois le même chantier `desentupir canos` (T1-MONEY CPC=6.51€ vol=1300 score=8463) sous un nouvel id `t_8e6591ba`. Brief strictement identique à `t_06389a27` (livraison PR #261 DRAFT, 04/08 09:53) et à `t_8da1e3d4` (NO-OP légitime déjà consigné L2044-2058). Le présent worker a été spawné sur la branche `feat/cnr-rankpush-canalizador-urgente-t_f3259fea` (la même que celle du NO-OP `t_f3259fea` voisin).
- **Diagnostic re-validation live** :
  - PR #261 toujours OPEN / DRAFT sur `feat/cnr-rankpush-desentupir-canos-v2-t_06389a27` (synchro origin OK, `gh pr view 261` = state=OPEN, head=feat/cnr-rankpush-desentupir-canos-v2-t_06389a27)
  - commit `158557163` (rank-push canos) **déjà présent** sur la branche courante `feat/cnr-rankpush-canalizador-urgente-t_f3259fea` (`git log --oneline main..HEAD` ligne 6)
  - donc : la branche courante EST une ancêtre de la branche du PR #261 — toute tentative de re-push créerait une collision de branche et un force-push destructeur (R6 interdit)
  - le diff `main..HEAD` (+1048 lignes, 8 fichiers) est entièrement composé des NO-OP successifs historisés : `158557163` (rank-push initial canos, déjà dans PR #261), `4f3e99e9e` (rank-push `desentupimento`, chantier voisin PR #260), `2e4e2ae8b`/`7a1f4eb2d`/`9883ccf27`/`ee0d51c0b` (consignations doc-only NO-OP)
  - le `git status` montre un diff résiduel sur `guia-desentupir-canos.html` (échappement `\/` → `/` dans JSON-LD FAQPage) — **non lié à ce chantier**, effet d'un autre worker parallèle (`t_7221c8c6` lava-loica probablement), hors périmètre strict
  - vérif live `https://canalizador-norte-reparos.pt/blog/guia-desentupir-canos` : HTTP/2 200 ✓, mais title encore ancien (« Causas, Prevenção e Quando Não Tentar Sozinho ») — preuve que PR #261 est NON mergée (R7 respectée)
- **Décision** : **NO-OP légitime** (3e du nom pour ce chantier, après `t_8da1e3d4` et le re-push initial `t_06389a27`). Leçon #469 (anti-doublon cache-lag) s'applique strictement. Tenter le livrable violerait R6 (force-push destructeur sur branche partagée), R7 (réouvrirait une PR identique qui ne peut que diverger), et l'esprit de la doctrine (renforcement > duplication).
- **Action réalisée sur ce run** : (1) re-validation live 5 invariants (PR #261 OPEN, branche ancêtre, commit déjà présent, JSON-LD conforme post-rebase, interdits R5/R11 vérifiés) ; (2) consignation traçable dans ce `SEO_PLAN.md` (présente entrée) ; (3) `kanban_complete` sans re-push Git, sans toucher au diff résiduel `guia-desentupir-canos.html` (hors périmètre) ; (4) branche `feat/cnr-rankpush-canalizador-urgente-t_f3259fea` intacte.
- **Recommandation pool-keeper** : cesser la réinjection de ce chantier tant que PR #261 n'est pas mergée/rejetée. Le pool-keeper doit implémenter un **filtre anti-réinjection** sur l'id de tâche `t_06389a27` + ses dérivés (`t_8da1e3d4`, `t_8e6591ba`) tant que la PR reste OPEN. Tant que GO Filipe n'est pas donné, la création d'une nouvelle tâche dérivée viole R6/R7.
