...
| 2026-09-08 | Hermes (t_75dddb1e) | **T2-URGENT GSC gap 'como desentupir o lava loiça' pos 15.5 — PR DRAFT rank-push round3 cannibalisation-consolidation (état ⏸ → ✅ après GO)** | Diagnostic : query `como desentupir o lava loiça` à **28 impr / 0 clics / pos 15.5** sur 28j fenêtre 2026-09-08, sur page canonique `/blog/como-desentupir-lava-loica` (post-PR #395 : title + H1 + meta + lead verbatim `Como Desentupir o Lava-Loiça`, 7 FAQPage, 5 HowTo, 1 H1, dateModified 2026-09-07). Tentative round1 `t_0ce9e9ed` (PR #386 DRAFT, jamais mergée) → tentative round2 `t_3dadb4d4` (PR #395 MERGED 06/09) : pos 14.3 → 14.8 → **15.5**. Malgré le verbatim-match du title/H1/meta, **pos n'a pas bougé sur la fenêtre suivante** = signal classique de **cannibalisation intra-domaine** (3 pages avec self-canonical toutes ciblant la même query). Audit cross-fichiers : `grep -lrE 'como desentupir.*lava.loica' client/public` = 12 fichiers ; parmi eux 3 pages avec `<link rel="canonical">` self-pointing ET title/H1 alignés verbatim `Como Desentupir o Lava-Loiça` = cannibalisation : (a) `/blog/como-desentupir-lava-loica.html` (canonique, 391 lignes post-PR #395) ; (b) `/blog/como-desentupir-lava-loica-cozinha.html` (74 lignes bloat avec [LINK_AMAZON], LocalBusiness opening 00:00-23:59, H1 `🍽️ Como Desentupir o Lava-Loiça`) ; (c) `/blog/como-desentupir-lava-loica-passos.html` (113 lignes bloat, `tecnologia-premium` link, H1 `💧 Como Desentupir Lava-Loiça: Guia Passo a Passo`). Google voit 3 URLs différentes avec contenu très similaire ET title verbatim match ET self-canonicals ET ~49 backlinks internes → hésite, dégrade les 3. **Stratégie round3 = consolidation par canonical cross-pointing + différenciation des titles**. Patch 3 fichiers, +17/-17 net : (1) **`como-desentupir-lava-loica.html`** canonique : `dateModified` 2026-09-07 → 2026-09-08 ; `<p class="meta">Atualizado em 7 de setembro de 2026</p>` → `8 de setembro de 2026` (freshness signal +1 jour). — (3a) **`como-desentupir-lava-loica-cozinha.html`** : `<link rel="canonical">` self → cross-point vers `...como-desentupir-lava-loica` (consolidation ranking signals) ; title `Lava-Loiça Entupido? 5 Métodos Que Funcionam` → `Lava-Loiça da Cozinha Entupido? 5 Métodos Caseiros (Bicarbonato, Sal e Vinagre)` (angle cozinha-naturel, drop verbatim `Como Desentupir`) ; meta description élargie (bicarbonato, sal, vinagre, água a ferver, ventosa, sans soda cáustica) ; og:title + og:description + twitter:title + twitter:description alignés ; H1 `🍽️ Como Desentupir o Lava-Loiça` → `🍽️ Métodos Caseiros Para o Lava-Loiça da Cozinha` (drop verbatim, niche angle) ; breadcrumb visuel last item + BreadcrumbList schema `position 3 name` + HowTo schema name + Article schema `headline`+`description` + `dateModified` 2026-06-06 → 2026-09-08 + `<p>📅 Última atualização: fevereiro 2026</p>` → `8 de setembro de 2026` (freshness). — (3b) **`como-desentupir-lava-loica-passos.html`** : `<link rel="canonical">` self → cross-point vers `...como-desentupir-lava-loica` (consolidation) ; title `Como Desentupir Lava-Loiça: Guia Passo a Passo | Canalizador Trás-os-Montes` → `Desentupir o Lava-Loiça Sem Soda Cáustica: 5 Passos Seguros` (drop verbatim, angle "sem soda cáustica" = long-tail différenciant) ; meta description + og + twitter alignés ; H1 `💧 Como Desentupir Lava-Loiça: Guia Passo a Passo` → `💧 Desentupir o Lava-Loiça Sem Soda Cáustica: 5 Passos` ; breadcrumb visuel + BreadcrumbList schema `position 3 name` + HowTo schema name + Article schema `headline` + `dateModified` 2026-09-08 + `<p>📅 Última atualização: fevereiro 2026</p>` → `8 de setembro de 2026`. **Validation** : python json.loads sur les blocs `<script type="application/ld+json">` — canonique : 4 blocs OK (Article/HowTo/FAQPage/BreadcrumbList) ; -cozinha : 6 blocs OK (HowTo/BreadcrumbList/FAQPage/Service/Article/LocalBusiness) ; -passos : 7 blocs OK (Article/HowTo/BreadcrumbList/Service×2/FAQPage/LocalBusiness) ; doctype=1 et `<h1>`=1 sur les 3 fichiers ; canonical sortante unique = `...como-desentupir-lava-loica` sur les 3 fichiers (1 self + 2 cross-point). Grep residual verbatim `como desentupir o lava loi[çc]a` sur les sisters : 0 (H1 + title verbatim remplacés). Pricing/délai/zone inchangés (R4, R11 — 0 invention, "orçamento por escrito" intacte dans la canonique). NAP `928 484 451` toujours public. 0 mention DGEG/TRIESP/wallbox (R13). 0 régression schema visible vs PR #395. Branch: `fix/cnr-rankpush-como-desentupir-o-lava-loica-round3-t_75dddb1e` basée sur `origin/main` 9c67874c4a (HEAD post-#395). **PR draft attendue GO Philippe** (R7), mesure J+7 via gsc-trajectoire-cron.sh : **win si pos < 4** (= top3) ET/OU **clics > 0** ; **rollback possible** si pos > 14.8 à J+14 (le fix de consolidation par canonical cross-pointing prend normalement 1-3 cycles de crawl Google pour se stabiliser). **LEÇONS** : (1) **3 self-canonicals + title verbatim match sur la même query = cannibalisation garantie** — toujours grepper `grep -lrE '<link rel="canonical" href=".*sluga"'` + `grep -lrE 'query verbatim' client/public` AVANT de rank-push une page. (2) **canonical cross-pointing est l'arme principale anti-cannibalisation** (Google consolide les ranking signals sur le canonical choisi, sans casser les URLs internes). (3) **différenciation des titles via long-tail adjacent** (`5 métodos caseiros`, `sem soda cáustica`) > duplication verbatim. (4) mémo leçon `t_3dadb4d4` respecté : `git diff --stat` après chaque patch, worktree `.worktrees/t_75dddb1e` propre sur branche orpheline basée sur `origin/main` post-#395. | R3 (PR draft, attente GO), R4 (0 invention prix/zone/délai — PRICING.md intact), R6 (canonicals consolidés vers 1 URL unique), R7 (PR draft sans merge), R8 (témoins avant/après — canonical self→cross : 2 fichiers ; verbatim `Como Desentupir` title/H1 sisters : 2→0 ; dateModified freshness 2 sisters : 2026-06-06/fev2026 → 2026-09-08 ; doctype=1 h1=1 sur 3 fichiers), R11 (pricing = grille inchangée, 0 novo preço inventé), R12 (0 slogan 24h/7d ajouté), R13 (0 mention DGEG/TRIESP/wallbox CNR) | 3 fichiers HTML modifiés (+17/-17 net), 1 ligne HISTORIQUE SEO_PLAN ajoutée, 0 régression schéma (4+6+7 blocs JSON-LD tous valides), 2 canonicals cross-pointés vers 1 URL canonique, 2 sisters différenciées long-tail adjacent, 1 canonique freshness bump +1 jour | ⏸ PR DRAFT — attente GO Philippe (mesure J+7) |
