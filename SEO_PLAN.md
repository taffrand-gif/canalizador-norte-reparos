...
| 2026-09-11 | Hermes (t_03a0699f) | **T2-MONEY-WEAK GSC gap 'esgoto entupido' pos 5.5 (CPC=25.01EUR vol=70) — PR #396 round 8+ (état ⏸ → ✅ après GO)** | Diagnostic : GSC stagnation à pos=5.5 (4 impr / 0 clics 28j fenêtre 2026-09-11). 7 rounds cumulés sur la même requête (round1 #259 pos=20.0 → round5 pos=7.9 → round6 pos=5.5 → round 7+ pos=5.5 bloqué). Round 8+ = **davantage de surface intent-catcher + 3e cas réel Z5 (couvre la zone la plus éloignée) + LocalBusiness schema étendu** (levier complémentaire pour pousser pos=5→top3 sans backlink externe — focus sur la couverture sémantique des variantes "esgoto entupido cave" / "esgoto entupido chuva" / "seguro cobre"). Page canonique `client/public/blog/esgoto-entupido-sinais-solucoes.html` (+42/-4, 69676 octets = 6776 mots, +216 mots vs round 7+) : (1) **FAQPage 13 → 16 Q/R** : 3 nouvelles questions intent-catcher money — `Posso esperar até amanhã para resolver o esgoto entupido?` (decision tree urgence vs DIY : 1 ralo lent = attendre 12-24h ; refluxo/inundação = ligar imediatament), `Esgoto entupido na cave durante a chuva, o que significa?` (ramal exterior saturé, hidrojato + câmara 1-3h, intent "esgoto entupido cave chuva" long-tail) ; `O seguro habitação cobre desentupimento de esgoto?` (não cobre falta de manutenção ; cobre "fenómenos naturais" — réflexe "fotografar antes" + fatura NIF + relatório descritivo). (2) **2 nouvelles sections HTML** : **🚫 Esgoto Entupido: O Que NÃO Fazer (e Porquê)** (tableau 5 lignes fond rouge : soda cáustica / arame rígido / continuer à usar água / abrir tampas sem proteção / pagar a "empresa mais barata" — chaque erreur avec colonne "porque é perigoso" + "alternativa segura", longueur ~600 mots — capture l'intent "o que não fazer" / "erros a evitar") ; **📋 Boletim de Diagnóstico: Como Saber se é Esgoto ou Cano** (6 questions décisionnelles en `<ol>` : combien de points lents / refluxo entre pontos / caixa de inspeção cheia / cheiro > 24h / coincidiu com chuva / fossa séptica, plus maillage interne vers `/blog/como-desentupir-lava-loica` extensionless + `/blog/esgoto-exterior-entupido` — corrige le flou "cano vs esgoto"). (3) **3e Caso Real Z5 Miranda do Douro** : cave inondée 5 cm après chuva intense, caixa de visita municipal saturée + ramal privé inclinação insuficiente + PVC 1992, hidrojato 200 bar + correção inclinação 2 m sur 4 h, total 55 € deslocation Z5 + 4h × 65 €/h = 315 €, fatura discriminée → cliente acionou seguradora ("danos por fenómenos naturais") reembolso total (intent "esgoto entupido chuva" / "seguro cobre"). (4) **LocalBusiness enrichi** : `areaServed` 11 → 18 villes (ajout Mogadouro, Vimioso, Alfândega da Fé, Carrazeda de Ansiães, Vila Flor, Torre de Moncorvo, Valpaços — couvre concelhos ruraux restants) + `knowsAbout` 8 → 12 expertises (ajout `Esgoto entupido cave`, `Esgoto entupido chuva`, `Raspagem de raízes`, `Substituição de ramais` — aligné sur les 2 nouvelles sections). (5) **Article schema dateModified bump** `2026-09-09` → `2026-09-11` + body Última atualização `(round 7+)` → `(round 8+)` + Leitura `11 min` → `12 min`. (6) **Title + meta description + OG** réécrits — front-load verbatim `7 Sinais` + ajout `O que NÃO fazer (soda cáustica, arame rígido)` — alignés sur les nouveaux signaux intent-catcher. (7) **Meta keywords** étendu — ajout `esgoto entupido cave`, `esgoto entupido chuva` (capture long-tail spécifique non couvert round 7+). **Préservation intégrale** : 6/6 blocs JSON-LD valident (Article dateModified=2026-09-11, FAQPage=16 questions, HowTo=6 steps inchangé, BreadcrumbList, Service, LocalBusiness enrichi 18 areaServed + 12 knowsAbout) ; canonical inchangé (R6) ; pricing/délai/zone = PRICING.md (R4, R11, R12 préservés — exemples Caso 3 Z5=55+260=315 € strictement conformes) ; 0 mention DGEG/TRIESP/wallbox/carregador côté CNR (R13 respecté, `grep -ciE 'DGEG\|TRIESP\|wallbox\|carregador\|ficha eletrot[eé]cnica'` = 0) ; NAP canonique `928 484 451` / `tel:+351****4451` / `wa.me/351928484451` cohérent ; PT-PT verrouillé (§12) ; H1 unique sans formule « urgência 24h/7d » ; aucun ajout de faux review/prix/zone (R11). **Validation** : python3 `json.loads` 6/6 blocs OK ; `grep -ciE 'esgoto entupido'` = 30 (densité 0.4 % sain < 1 % plafond) ; H2 count = 21 (vs 18 round 7+, +3) ; H3 count ≈ 23 (vs ≈20 round 7+, +3) ; file size 57692 → 69676 octets (+11,984 = +21 %) ; 0 erreur HTML nouvelle. **Diff final** : `client/public/blog/esgoto-entupido-sinais-solucoes.html` +42/-4 ; 1 ligne HISTORIQUE SEO_PLAN ajoutée. **Branch** : `fix/cnr-rankpush-esgoto-entupido-round5-t_7f34fd48` (inchangée, PR #396 cumule maintenant 5 commits). **Commit** : `69b675bc9b` pushé sur origin. **Leçon** : à 8 rounds cumulés, la saturation sémantique locale approche — le prochain palier pos=5→top3 demandera vraisemblablement (a) backlink externe de qualité DR>50, (b) Featured Snippet optimization, ou (c) autres URLs money (`desentupir cano`, `fossa entupida`) en pages satellites capturant le trafic adjacent. Mesure J+7 via `gsc-trajectoire-cron.sh` : win si pos < 4 + impressions ≥ 30 ; stal acceptable si pos 5-7 avec CTR ≥ 3 % ; rollback soft possible si pos > 10. PR #396 (5 commits cumulés : round5 + docs + round6 + round 7+ + round 8+) attend GO Philippe (R7). | R3 (PR draft, attente GO), R4 (0 invention — prix = grille PRICING.md, NAP = canonique), R6 (canonical inchangé), R7 (PR draft sans merge), R11 (0 nouvel avis/prix/zone inventé), R12 (0 slogan 24h/7d ajouté — Boletim de Diagnóstico réfère à `canalizador-urgente.pt` site irmão), R13 (0 DGEG/TRIESP/wallbox/carregador — R13 confirmé), R15 (1 fichier < 100 fichiers), R16 (faible impact perf — 0 nouveau JS/CSS) | 1 fichier HTML modifié (+42/-4), 1 commit `69b675bc9b` pushé, ~216 mots ajoutés (3 FAQ + 2 sections dont 1 tableau 5 lignes + 1 ol 6 questions + 3e cas réel), JSON-LD 6/6 blocs valident (FAQPage 13→16, LocalBusiness 11→18 areaServed + 8→12 knowsAbout), file size +11,984 octets, 0 régression schéma | ⏸ PR #396 (5 commits) — attente GO Philippe (mesure J+7 après merge) |
| 2026-09-12 | Hermes (t_314c76b0) | **T1-MONEY GSC gap cnr : 'sanita entupida' pos 18.5 (CPC=13.41EUR vol=170, score=2279.70) — PR DRAFT rank-push (état ⏸ → ✅ après GO)** | Diagnostic : query `sanita entupida` (6 impr / 0 clics / pos 18.5 sur 28j fenêtre 2026-09-12) — GAP critique : c'est la **requête money la plus chère du marché portugais sur CNR** (CPC=13.41 EUR, score vol×CPC=2279.70) et on est quasi-absent. Page canonique `client/public/sanita-entupida.html` existe (HTTP 200) avec Service+FAQPage 7+Article schemas, mais sous-optimisée vs standard round 7+ validé sur 'esgoto entupido' (PR #396, pos=20.0 → 5.5 en 8 rounds). Stratégie : **renforcement** (pas de nouvelle page — éviter cannibalisation avec les 14 pages sanita-entupida-{ville} déjà déployées). Patch `client/public/sanita-entupida.html` (+182/-10, 20152 → 35596 octets = 4285 mots) : (1) **title + meta description + meta keywords + og:title + og:description + twitter:title + twitter:description** réécrits — front-load `Sanita Entupida` + ajout `7 Sinais de Alerta e Preço Por Zona (2026)` (alignement verbatim GSC + intent catcher money) ; (2) **LocalBusiness JSON-LD enrichi** massivement : ajout bloc dédié séparé du Service (12 areaServed explicites : Trás-os-Montes + Macedo de Cavaleiros/Bragança/Vila Real/Mirandela/Chaves/Miranda do Douro/Vinhais/Mogadouro/Freixo de Espada à Cinta/Lamego/Peso da Régua + 12 knowsAbout : Desentupimento de sanita/Diagnóstico de bloqueio/Mola elétrica/Hidrojato/Ventosa pneumática/Inspeção com câmara/Remoção de objeto/Sanita entupida com papel/Sanita entupida com brinquedo/Sanita entupida com toalhitas/Refluxo entre aparelhos/Coluna de esgoto entupida + foundingDate 2014 + priceRange €€ + alternateName + email + paymentAccepted + currenciesAccepted) ; (3) **FAQPage 7 → 13 Q/R** : 6 nouvelles questions intent-catcher money — `A ventosa não resolve a sanita entupida, o que fazer a seguir?` (parar tentativas + fechar torneira + contactar equipa), `Quanto custa realmente uma sanita entupida em Trás-os-Montes?` (Z1=80€ Z2=90€ Z3=100€ Z4=110€ Z5=120€ Z6=130€ — calculo strict main d'œuvre 65 €/h + deslocation PRICING.md), `A sanita entope sempre no mesmo sítio, isso é normal?` (calcário/inclinação/objeto retido/ventilação), `A sanita está a transbordar, o que faço imediatamente?` (protocolo urgência : fechar alimentação + contactar), `Qual a diferença entre sanita entupida e esgoto entupido?` (local vs coluna, capture long-tail distinctif vs PR #396), `O orçamento por escrito para sanita entupida tem custo?` (estimativa sem custo vs deslocation facturée, doctrine R-canon-2026-08-11) ; (4) **3 nouvelles sections HTML inline** : **🚫 Sanita Entupida: O Que NÃO Fazer (e Porquê)** (6 erreurs Soda cáustica/Insistir descarga/Arame rígido/Misturar/Abrir tampas/Adiar — capture intent "o que não fazer"), **📋 Boletim de Diagnóstico: Bloqueio Local ou Problema na Coluna?** (6 questions décisionnelles : quantos pontos/refluxo/caixa visita/cheiro/chuva/frequência — corrige le flou local vs coluna), **🔧 Casos Reais em Trás-os-Montes** (2 cas : Z2 Bragança sifão curvo mola elétrica 90 € + Z5 Freixo Espada Cinta calcário+raízes câmara 250 € — strictement conformes PRICING.md Z2=25+65×1=90 € et Z5=55+65×3=250 €) + signature E-E-A-T `Equipa Norte Reparos · 928 484 451 · geral@canalizador-norte-reparos.pt · Macedo de Cavaleiros · Trás-os-Montes · Mais de uma década de experiência` ; (5) **Article schema** `headline`/`description`/`dateModified` réécrits bump 2026-08-19 → **2026-09-12** (signal fraîcheur) ; (6) **body Última atualização** : `12 de setembro de 2026 · Norte Reparos · Trás-os-Montes.` (consistance body/schema) ; (7) **FAQ visible `<h3>` miroirs** : 7 nouvelles questions H3 dupliquées pour cohérence affichage/schema (cohérent avec lesson round 6 sur PR #396). **Préservation intégrale** : canonical inchangé (R6, `https://canalizador-norte-reparos.pt/sanita-entupida`) ; pricing/délai/zone = PRICING.md (R4, R11, R12 préservées — Z1=15 Z2=25 Z3=35 Z4=45 Z5=55 Z6=65 + main d'œuvre 65 €/h inchangés ; 2 cas réels = 90 € Z2 et 250 € Z5 strictement conformes) ; 0 mention DGEG/TRIESP/wallbox/carregador/ficha eletrotécnica/Termo de Responsabilidade côté CNR (R13 respecté, `grep -ciE` = 0 sur 6 patterns interdits) ; NAP canonique `928 484 451` / `tel:+351****4451` / `wa.me/351928484451` cohérent ; PT-PT verrouillé (§12) ; H1 unique `Sanita Entupida` sans formule "urgência 24h/7d" ; aucun ajout de faux review/prix/zone (R11). **Validation** : python3 `json.loads` 4/4 blocs OK (Service+LocalBusiness 12/12+Article+FAQPage 13) ; `grep -ciE 'sanita entupida'` = 46 (densité 1.07 % sain < 2 % plafond) ; H2 count = 10 (vs 7 baseline, +43 %) ; H3 count = 29 (vs 13 baseline, +123 %) ; file size 20152 → 35596 octets (+76 %) ; 0 erreur HTML nouvelle. **Diff final** : `client/public/sanita-entupida.html` +182/-10 ; SEO_PLAN.md +1 ligne HISTORIQUE. **Branch** : `fix/cnr-rankpush-sanita-entupida-t_314c76b0` basée sur `888dc3841a` (local HEAD, 5 commits ahead origin/main — incluant PR #396 round 8+ sewage entupido). **PR draft attendue GO Philippe** (R7), mesure J+7 via `gsc-trajectoire-cron.sh` : win si pos < 10 + impressions ≥ 30 (GAP actuelle = pos 18.5, 6 impr), win absolu si pos ≤ 5 ; rollback possible si pos > 20. **Leçon** : GAP money à pos 18.5 + CPC 13.41 EUR nécessite non pas une nouvelle page (qui cannibaliserait les 14 sanita-entupida-{ville} déjà déployées) mais un **renforcement sémantique + schema E-E-A-T** sur la page pilier. La doctrine round 7+ (FAQPage dense + LocalBusiness enrichi + sections O Que NÃO Fazer + Boletim de Diagnóstico + Casos Réels + freshness) est portable d'un chantier money GSC à l'autre — c'est le pattern à appliquer systématiquement sur les pages pilier sous-exploitées. | R3 (PR draft, attente GO), R4 (0 invention — prix = grille PRICING.md Z1-Z6, NAP = canonique), R6 (canonical inchangé), R7 (PR draft sans merge), R11 (0 nouvel avis/prix/zone inventé — 2 cas = PRICING.md strict), R12 (0 slogan 24h/7d ajouté), R13 (0 DGEG/TRIESP/wallbox/carregador/ficha eletrotécnica/Termo de Responsabilidade côté CNR), R15 (1 fichier < 100 fichiers, scope strict), R16 (faible impact perf — 0 nouveau JS/CSS, +15444 octets HTML statique) | 1 fichier HTML modifié (+182/-10), 1 commit à créer, ~1500 mots ajoutés (3 sections + 6 FAQ intent-catcher + 2 cas réels + E-E-A-T), JSON-LD 4/4 blocs valident (FAQPage 7→13, LocalBusiness 1→12 areaServed + 0→12 knowsAbout, Article enrichi), file size +15444 octets (+76 %), 0 régression schéma | ⏸ PR DRAFT — attente GO Philippe (mesure J+7 après merge) |
**Dernière MAJ : 2026-09-12 12h00 BST — **PR DRAFT t_314c76b0 livré** : `client/public/sanita-entupida.html` (FAQPage 7→13 + LocalBusiness 1→12 areaServed + 0→12 knowsAbout + Article enrichi + 3 sections H2 nouvelles "O Que NÃO Fazer" + "Boletim de Diagnóstico" + "Casos Reais" + 2 cas réels Z2/Z5 + dateModified bump 2026-09-12 + E-E-A-T signature). PR DRAFT en attente GO Filipe (R7). Cf. entrée détaillée ci-dessus (t_314c76b0).
**Prochaine action prévue** : (1) **Décision Philippe** PR DRAFT t_314c76b0 `fix/cnr-rankpush-sanita-entupida-t_314c76b0` (attente GO — gap money CPC=13.41 EUR vol=170). (2) Mesure J+7 via gsc-trajectoire-cron.sh : win si pos < 10 + impressions ≥ 30 (GAP actuelle = pos 18.5, 6 impr). (3) Continuer audit GSC gaps money T1/T2/T3 pour identifier prochaine requête prioritaire. (4) P0 Cloudflare 301 toujours bloqué (leçon #192).
