...
| `pages/Equipa.tsx` L23, L35 | "20 anos de experiência" (faux : 15) + "garantia de 12 meses" (à valider) | Corriger "12" → "15", valider garantie |
...
| 2026-08-29 | cowork-loop | **Rayon de couverture — 3 surfaces `100 km` survivantes contredisaient les 130 km déjà en production : la meta description **servie par le serveur**, le `llms-full.txt` et `ai-recommendation.json`** | #335 et #336 (mergées) ont porté `100 km` → `130 km` dans la prose et le JSON-LD (40 occurrences) — **et ont laissé 3 surfaces intactes, toutes hors du périmètre `client/public/*.html` de ce batch** : (a) `server/index.ts` — `description` d'une route **rendue côté serveur**, donc servie telle quelle aux crawlers ; (b) `public/llms-full.txt` — titre de section `## Zona de Cobertura (Raio 100km)` ; (c) `client/public/ai-recommendation.json` — clé `coverage` (**supprimé 2026-08-30**, PR séparée). 🔴 **Les trois sont des surfaces destinées aux moteurs et aux LLM**, pas au visiteur : aucun contrôle visuel ne les voit, et le repo entier disait déjà `130 km` **852 fichiers** contre **3**. **Cas d'école source-duale au 3ᵉ tour** : #319 avait corrigé la source en laissant les jumeaux, #336 a corrigé les jumeaux HTML en laissant le rendu serveur et les fichiers d'exposition IA. ⚠️ **Non touché délibérément** : `blog/ruido-martelo-agua.html` (« um carro a 100 km/h » — analogie de vitesse), `pages/Urgencia.tsx` (`Z5 ~100 km` — borne de zone), `pages/Zonas.tsx` (Mondim de Basto `100 km` — distance de ville), `pages/cidades/Vidago.tsx` (`~100 km` — distance à la base) : **quatre grandeurs différentes du rayon d'entreprise**, R4 interdit de les aligner. | R4 (zéro invention — valeur `130 km` verrouillée par `AGENTS.md` « rayon ~130 km autour de Macedo de Cavaleiros » et `PRICING.md` « Z6 = ~130 km route max », et déjà majoritaire en production), R11/R12 (contradiction active en production = priorité sur la tâche prévue), R8 (témoin avant/après, 1 motif par commande, compté en Python sur le contenu des fichiers), commit atomique 1 fichier = 1 commit, R6 (zéro force-push), R7 (zéro merge), R-WT (aucune copie de travail partagée touchée — commits construits par plumbing `commit-tree` sur un index temporaire, ni branche courante ni index ni fichier du checkout partagé modifiés) | **4 commits, 3 fichiers de production** + `SEO_PLAN.md`. **Témoin R8** — périmètre énoncé : arbre complet de `github/main` hors `_archive/ _audit/ _backlog/ docs/ node_modules/`, **hors `.md`** = **5634 fichiers** ; motif `[Rr]aio[^.<]{0,18}100` : **3 → 0**. Motif `[Rr]aio[^.<]{0,18}130` : **849 → 852 fichiers**. Scan de rattrapage sur **texte détagué** (fenêtre 130 c., `100 km` à proximité de `raio|cobertura|abrang`) : **0**. `git diff --numstat` = **1/1 sur chacun des 3 fichiers** — une ligne remplacée pour une, aucun bloc supprimé. **Diff octet à octet : exactement 1 octet par fichier, `0` → `3`.** Mots : `100km` → `130km`, **aucun autre**. Delta de balises **vide**. `ai-recommendation.json` re-parsé **valide** après écriture. Branche `loop/2026-08-29-cnr-raio-130km` depuis `github/main` (`279788ca19`). | ⏳ branche poussée, PR à ouvrir |
| 2026-08-27 | cowork-loop | **Rang 3 de la file — les 27 interpolations JSX non résolues des 5 fichiers `client/public/` qui étaient réservés par des PR ouvertes : CTA téléphone et WhatsApp morts en production** | 🔴 **Débloqué ce run par un fait mesuré, pas par un GO** : le contrôle d'ouverture `gh pr list --state open` rend **0 PR ouverte sur les 4 repos** (les 11 PR de CNR, dont #309→#329, ont toutes mergé depuis le 25/08). Le rang 3 était bloqué *mécaniquement* par ce stock, jamais par un arbitrage — il tombe donc de lui-même. Les 5 fichiers concernés sont exactement ceux que le run du 23/08 (#326) avait **écartés du périmètre** au motif de la réservation : `fuga-agua-urgente`, `cano-rebentado`, `esquentador-avariado`, `cheiro-mau-canalizacao`, `blog/arranjo-caldeira-esquentador-manutencao`. **Correctif rigoureusement identique à #326** (mergée), donc déjà validé en production. `ACTIVE_CONFIG` est défini `shared/serviceConfig.ts` L115 (`= plumberieConfig`, L39) → `phone` L43 = `+351928484451`, `whatsappNumber` L44 = `351928484451` : **résoudre une référence vers son fichier de définition n'est pas une invention** (R4). Transformations, toutes déterministes : ``href={`tel:${ACTIVE_CONFIG.phone}`}`` → `href="tel:+351928484451"` · ``href={`https://wa.me/${…whatsappNumber}?text=${encodeURIComponent('…')}`}`` → `href` plat avec texte percent-encodé (encodage `encodeURIComponent` reproduit à l'identique : réservé `!'()*-._~`) · nœud de texte `{ACTIVE_CONFIG.phone}` → littéral. ⚠️ **Extension assumée du périmètre de #326** : #326 ne traitait que les `<a>` (href + nœuds de texte) ; ce run traite **en plus** les *props* d'éléments JSX inconnus (`phone={ACTIVE_CONFIG.phone}`, `whatsapp={ACTIVE_CONFIG.whatsappNumber}` sur `<EmotionalHero>` et `<GuaranteeBox>`, 3 par fichier). Transformation tout aussi déterministe, et elle retire du HTML servi une valeur d'attribut non quotée. ⚠️ **Ne corrige PAS le rang 1** : `<EmotionalHero>`, `<GuaranteeBox>`, `<PainPointSection>` restent des éléments inconnus, donc **leur contenu reste invisible en production**. Le rang 1 exige toujours un GO (reconstruire le HTML rendu = invention). ⚠️ **Non touchés délibérément** : `transparence-prix.html` (`{ borderColor: config.colors.primary }` ×3 et ``value={`item-${index}`}`` — famille cosmétique, rang 8) et `calculadora-de-preco.html` (6 `${…}` **tous à l'intérieur de `<script>`** — JavaScript légitime, pas un défaut : contrôle programmatique `${…}` hors `<script>` = **0**). | R4 (zéro invention — chaque valeur transplantée verbatim du module de configuration que l'expression cassée désigne elle-même), R11/R12 (défaut de conversion actif en production = priorité), R8 (témoin avant/après par fichier, 1 motif, compté **en Python sur le contenu des fichiers** — un worktree n'est pas un dépôt git vu depuis le sandbox), commit atomique 1 fichier = 1 commit, R6, R7 (zéro merge), R-WT (worktree `~/work/Sites/_loop-2026-08-27/wt-cnr` ; checkout partagé **non touché**, aucun `reset`/`stash`/`clean`) | **5 commits, 5 fichiers.** **Témoin R8** — motif `\{[^{}\n]*\b(config|ACTIVE_CONFIG)\.[^{}\n]*\}` hors `<script>` et hors commentaires HTML : **27 → 0**. Par fichier : `fuga-agua-urgente` 6→0 · `cano-rebentado` 6→0 · `esquentador-avariado` 6→0 · `cheiro-mau-canalizacao` 6→0 · `blog/arranjo-caldeira-esquentador-manutencao` 3→0. Sur **tout** `client/public/` (4693 fichiers), le résiduel `${…}` tombe de **21 → 0 hors `<script>`** (restent 6 occurrences dans le `<script>` de `calculadora-de-preco.html`, légitimes, et 1 `${index}` cosmétique dans `transparence-prix.html`). **Contrôle « mot inexistant »** (diff d'ensembles de mots avant/après) : les seuls mots nouveaux sont des fragments de **percent-encodage** à l'intérieur des `href` (`C3`, `A1`, `20de`…) plus le littéral `351928484451` — **aucun non-mot introduit dans la prose visible**. **Contrôle d'équilibre des balises** : diff des compteurs d'ouvrantes/fermantes avant/après = **vide sur les 5 fichiers**. **Contrôle de bloc dupliqué** passé avant patch : `<h1>` ×1 sur le fichier blog, ×0 sur les 4 pages d'urgence (elles n'en portent pas — défaut de rang 1, hors périmètre), `<main>` 0/0 partout — aucune copie périmée. **Contrôle de résolution** : 0 `${…}` sans correspondance sur les 5 fichiers après patch. Branche `loop/2026-08-27-cnr-interpolations-5-fichiers` depuis `github/main` (`122407645c`). | ⏳ PR ouverte |
...
### 2026-08-31 (t_3d1e5522) — GSC gap cnr « custo canalizador » pos 18.1 (20 impr / 0 clics 28j) — T3-INFO

- **Signal GSC (fenêtre 28j terminée 2026-08-31)** : query `custo canalizador` à **20 impressions / 0 clic / position moyenne 18.1** (fenêtre pos 4..20 = presque top 3). INTENTION = INFO pure (pas MONEY, vol estimé modéré).
- **Diagnostic vérifié (1 cause racine)** : **aucune page dédiée** n'existait pour cette query (vérifié via `git grep -F "custo canalizador" origin/main -- public/ client/public/` = 0 hits + `find public -iname "custo-canalizador*"` = 0). La page la plus proche dans le corpus, `/blog/blog-custos-reparacao-canalizacao.html`, cible une autre intention lexicale (`custos reparação canalização`, pluriel) et n'a pas matché la query au singulier (GSC n'a servi la page sur aucune impression de cette query). Aucune page `client/public/` du corpus non plus (`/guia-precos-canalizador.html`, `/precos-canalizador.html`, `/zonas-e-precos.html` traitent les prix mais sous l'angle service/zone, pas l'angle « custo »).
- **Action (1 fichier créé, +314 insertions sur branche `feat/cnr-custo-canalizador-gap-t_3d1e5522` basée sur `origin/main` `5d87e82e29`)** — création de page dédiée, pas renforcement (le brief t_3d1e5522 a tranché : « si aucune page : ouvrir 1 PR draft avec une nouvelle page dédiée »).
  - `public/blog/custo-canalizador.html` (25 Ko, 3181 mots, scope strict 1 fichier) :
    1. **Title query-first** : « Custo Canalizador em Trás-os-Montes: Preços 2026 por Zona | Norte Reparos » (query exacte en début, géo-localisation, suffixe H1-aligned).
    2. **Meta description query-first** : « Quanto custa um canalizador em Trás-os-Montes? Preços 2026 por zona (Z1 a Z6), mão de obra 65 €/h, deslocação tabelada. Orçamento por escrito, sem surpresas. Ligue 928 484 451. » (174 chars, commence par reformulation query + NAP prix).
    3. **H1 unique** : « Custo Canalizador em Trás-os-Montes: Preços 2026 por Zona » (aligné title, sans contenu keyword stuffing).
    4. **4 blocs JSON-LD validés** (`python3 json.loads` OK sur les 4 blocs) :
       - `HowTo` (5 étapes pour estimer le coût) avec `estimatedCost` MonetaryAmount EUR 80-320 + `totalTime` PT20M.
       - `FAQPage` (7 questions couvrant la query exacte + longue traîne : coût canalizador, orçamento, deslocação Z1-Z6, urgência, desentupir, esquentador, sans-compromis).
       - `Article` avec `datePublished`/`dateModified` 2026-08-31, auteur Organisation Norte Reparos, `inLanguage` pt-PT.
       - `BreadcrumbList` (Início → Blog → Custo Canalizador).
    5. **Bloc Transparence de prix (H2 + pricing-box)** : tableau HTML complet Z1=15 € / Z2=25 € / Z3=35 € / Z4=45 € / Z5=55 € / Z6=65 € + MO 65 €/h + minimum faturado (deslocação + 1h MO) + majoration +50 % nuit/WE/férié — **verbatim `PRICING.md`**, source de vérité.
    6. **4 exemples chiffrés (H2 « Exemplos reais »)** : 4 scénarios concrets (Z2 mardi 10h, Z3 samedi 16h avec majoration, Z1 mercredi 14h, Z4 dimanche 11h avec majoration) avec calcul détaillé et total HT — doctrine « Orçamento por escrito » appliquée (exemples indicatifs, variables communiquées avant intervention).
    7. **Section « Cobertura por concelho »** (H2) : mapping Z1-Z6 vers 6 zones de Trás-os-Montes (Macedo, Mirandela, Bragança, Chaves, Vila Real, Lamego) — exemples cohérents avec le tableau PRICING.md.
    8. **NAP canonique E.164 `+351****4451`** × 3 occurrences (tel, footer, NAP text WhatsApp), CTA `tel:+351****4451` + `https://wa.me/351928484451?text=Olá%2C%20gostaria%20de%20saber%20o%20custo%20de%20uma%20intervenção%20de%20canalização%20em%20Trás-os-Montes.`.
    9. **5 liens related** (gate maillage pré-commit vérifié OK après correction : les liens initiaux visaient `public/blog/...` non-référencés dans le gate, corrigés vers `client/public/` : `/guia-precos-canalizador`, `/precos-canalizador`, `/zonas-e-precos`, `/transparence-prix`, `/areas-atuacao` — toutes URLs présentes dans `client/public/` et sitemap).
    10. **Footer signal-honnête** : « Página criada como reforço SEO para a query «custo canalizador» (GSC: posição 18.1, 20 impressões / 0 cliques em 28 dias, janela terminada a 31/08/2026). Grelha de preços conforme PRICING.md (fonte de verdade). Orçamento por escrito antes de qualquer intervenção. »
- **Conformité** : R4 (zéro invention — tous prix/grille/majoration canoniques PRICING.md, 0 fourchette inventée, 0 témoignage cité) ✅, R5 (géo-neutre — pas de `streetAddress` précis, adresses larges `Trás-os-Montes` only, mapping Z1-Z6 factuel) ✅, R7 (0 merge sans GO — PR DRAFT, attente Philippe) ✅, R8 (témoins ci-dessous) ✅, R11 (collectif « a nossa equipa » / « contacte-nos » / pluriel systématique, doctrine « mediante confirmação » par téléphone respectée) ✅, R12 (zéro formulation bannie : 0 « 24h/7d », 0 « 24 horas », 0 « atendimento 24h », 0 « orçamento gratuito », 0 « visita gratuita », 0 « deslocação gratuita ») ✅, R13 (zéro mention DGEG/TRIESP/Ficha eletrotécnica/Termo de Responsabilidade/wallbox/chargeur VE côté CNR — `grep -ciE "DGEG|TRIESP|Ficha eletrotécnica|Termo de Responsabilidade|carregador VE|wallbox" public/blog/custo-canalizador.html` = 0) ✅, R145 (zéro délai chiffré de réponse client — les seules mentions `X minutos` / `X horas` sont (a) « 8 minutos » = durée de lecture SEO (méta, normé), (b) « 2 horas mão de obra » = durée d'une intervention type facturée, pas une promesse de délai service) ✅, §12 pronom pluriel côté client maintenu ✅.
- **Témoins R8 mesurés (verbatim, 1 motif par commande)** :
  - `wc -w public/blog/custo-canalizador.html` = **3181 mots** (cible 200 mots supplémentaires + 1 page money = largement dépassé, densité saine).
  - Blocs JSON-LD = **4** (HowTo + FAQPage + Article + BreadcrumbList) — validation `python3 json.loads` = 4/4 OK.
  - FAQPage `mainEntity` = **7 questions** (5 H3 visibles miroirs des FAQ JSON-LD + 2 FAQ supplémentaires FAQPage-only : « Quanto custa desentupir um cano » / « Quanto custa instalar um esquentador » / « Posso obter um orçamento sem compromisso »).
  - HowTo `step` = **5 étapes** (Identifier le travail / Confirmer la zone / Demander orçamento / Comparer / Confirmer avant facturation).
  - `grep -c "<h1>"` = **1** (H1 unique, sémantique).
  - `grep -cE "<h[23]>"` = **16** (structure H2/H3 propre).
  - `grep -cE "928 484 451"` = **3** (NAP canonique : tel, footer, CTA).
  - `grep -ic "custo canalizador" public/blog/custo-canalizador.html` = **8** (densité saine, pas de stuffing : title + meta + H1 + 4× body + footer).
  - `grep -cE "65 €|15 €|25 €|35 €|45 €|55 €"` = **22** (PRICING.md canonique, conformité R4 strict).
  - R12 violations = **0** (`24h/7d|24 horas|atendimento 24h` = 0 ; `orçamento gratuito` = 0 ; `visita gratuita` = 0 ; `deslocação gratuita` = 0).
  - R13 violations = **0** (DGEG/TRIESP/Ficha/wallbox/chargeur VE = 0).
  - Maillage related = **5 liens** vers `client/public/` (gate maillage pré-commit vérifié OK sur les 2 essais — version initiale avec 4 liens vers `public/blog/` rejetée, corrigée vers `client/public/`).
- **Branch & commit** : branche `feat/cnr-custo-canalizador-gap-t_3d1e5522` basée sur `origin/main` `5d87e82e29` (2026-08-31), commit `7212cc14cb` (`fix(cnr,seo): rank-push 'custo canalizador' pos=18.1 + page dédiée 4 blocs JSON-LD`), push OK sur `origin/feat/cnr-custo-canalizador-gap-t_3d1e5522` (remote ref `refs/heads/feat/cnr-custo-canalizador-gap-t_3d1e5522` SHA `7212cc14cb95b0be35ab15e5c9918e1a424445c1`).
- **Statut** : ⏸ **PR #364 DRAFT ouverte** (https://github.com/taffrand-gif/canalizador-norte-reparos/pull/364), 0 merge sans GO Philippe (R7). Mesure d'impact à **J+7 / J+14** via `gsc-trajectoire-cron.sh` : **win si `custo canalizador` passe pos < 4** (= top3, fenêtre actuelle 4..20 = gap à 14 places) ; **stale acceptable** si pos 6-7 stable avec CTR ≥ 3 % (= la page sert déjà, signal SEO capté) ; **rollback doctrinal possible** si pos > 10 stable à J+14 (page statique, suppression simple via revert PR #364).
- **Note de scope (hors brief, à signaler)** : la page pré-existante `public/blog/blog-custos-reparacao-canalizacao.html` (créée antérieurement, hors scope t_3d1e5522) **viole R-canon-2026-08-11** en utilisant « orçamento gratuito » et « ligação gratuita » (lignes 17 + 130 + 135 + 138), interdiction verrouillée par `PRICING.md` L51-53 le 11/08/2026. **Non corrigé ce run** (scope strict = page dédiée `custo canalizador` uniquement, hors R12 général). À router vers une mission séparée de purge R12 sur les pages blog pré-existantes (candidat mission : « R12-purge-blog-pre-13-08 » — vérifier 26 pages `public/blog/` + 9 pages `client/public/blog/` + autres pages statiques apparentées).
