...
| X-MAIL | Email `privaterelay.appleid.com` publié comme contact | HAUTE | A_FAIRE | — | — | `privaterelay\.appleid\.com` · **production CNR 3, dont 2 servis** — servis : `client/public/.well-known/ai-plugin.json` · `client/public/.well-known/security.txt`. NON servi : `public/.well-known/ai-plugin.json` (racine, voir X-PUB0). **Travail servi : 2 fichiers.** Contrôle positif `appleid` = 4. |
