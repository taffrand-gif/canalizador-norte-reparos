// Placeholder honnête — R11 ZÉRO INVENTION (témoignages fictifs purgés 2026-07-01)
// Voir `~/work/Sites/M5-AUDIT-AVIS-REELS-2026-07-01.md` pour l'audit complet.
import { useSite } from '@/contexts/SiteContext';

export default function GoogleReviews() {
  const { config } = useSite();
  const phoneDisplay = config.contact?.phoneDisplay ?? '+351 928 484 451';
  const phoneRaw = (config.contact?.phoneRaw ?? '351928484451').replace(/[^0-9]/g, '');
  const whatsappUrl = `https://wa.me/${phoneRaw}`;
  const sectionClass = 'py-16 bg-gray-50';

  return (
    <section className={sectionClass} aria-label="Avaliações de clientes">
      <div className="container mx-auto px-4">
        <div className="max-w-2xl mx-auto text-center bg-white rounded-2xl p-8 shadow-sm border border-gray-100">
          ...
          <p className="text-gray-600 mb-6 leading-relaxed">
            A nossa equipa prefere mostrar-lhe uma intervenção concreta em vez de estrelas
            ou citações inventadas. Após cada serviço, pedimos ao cliente que partilhe a
            sua experiência por WhatsApp — só publicamos quando temos confirmação real
            e autorizada.
          </p>
          <a
            href={whatsappUrl}
          ...
          <p className="text-xs text-gray-400 mt-4">
            Norte Reparos · {phoneDisplay} · Trás-os-Montes
          </p>
        </div>
      </div>
    </section>
