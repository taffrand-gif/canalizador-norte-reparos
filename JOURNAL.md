...
### 2026-09-05 — GSC rank-push « como desentupir o lava loiça » round 2 (Hermes t_0ce9e9ed) — T2-URGENT

- **Signal GSC (fenêtre 28j terminée 2026-09-05)** : query `como desentupir o lava loiça` à **38 impressions / 0 clic / position moyenne 14.3** (fenêtre pos 4..20 = presque top3, CTR 0 % = snippet pas incitatif). INTENTION = URGENT (info + signal money : 0 clic alors que pos 14 = pénalité SEO classique).
- **Diagnostic vérifié (1 cause racine)** : la page canonique `/blog/como-desentupir-lava-loica` existe et a été renforcée en août (PR #255 commit `398dfd5005`, puis merge rebase #261 commit `e8ade2901e` du 2026-08-17). **Mais** le merge rebase #261 a APPENDU en queue de fichier un second document HTML complet (lignes 358-455), avec son propre `<!DOCTYPE>` + `<head>` (title « Como Desentupir Lava Loiça: Passos Seguros e Práticos 💧 ») + `<body>` + `</html>`. Le navigateur parse le premier document et **ignore le second**. Conséquence : Google voit `<title>Desentupir Lava-Loiça: 5 Métodos Seguros (Passo a Passo 2026)</title>` — qui ne commence PAS par la query verbatim « como desentupir o lava loiça ». Le **bon** `<title>` (aligné query verbatim) était bien dans le repo, mais dans le document ignoré.
- **Action (1 fichier, +4/-104 sur branche `fix/cnr-rankpush-como-desentupir-lava-loica-t_0ce9e9ed` basée sur `github/main` 264bef01cc)** — renforcement chirurgical 4 modifs :
  1. **Title** : « Desentupir Lava-Loiça: 5 Métodos... » → « **Como Desentupir o Lava-Loiça**: 5 Métodos... » (préfixe « Como » verbatim query).
  2. **Meta description** : « Desentupir lava loiça em casa: 5 métodos... » → « **Como desentupir o lava loiça** em casa, do mais simples ao profissional: ventosa, sal, limpeza do sifão, máquina profissional e sinais de paragem. Guia PT-PT por canalizador em Trás-os-Montes. » (lead verbatim).
  3. **H1** : « Desentupir Lava-Loiça: 5 Métodos... » → « **Como Desentupir o Lava-Loiça**: 5 Métodos... » (H1 verbatim query).
  4. **Lead `<p>`** : `<strong>Desentupir lava loiça</strong>` → `<strong>Como desentupir o lava loiça</strong>` (bold verbatim).
  6. **Suppression lignes 358-455** : second document HTML concaténé (3ème `<head>`, 2ème `<body>`, 2ème `</html>` retirés). Fichier : 56787 → 22785 bytes (-60 %). Compteurs finaux : `<!DOCTYPE>` 2 → **1**, `</html>` 2 → **1**, `<head>` réel (sans compter `<header>`) 2 → **1**.
- **Périmètre (volontairement étroit)** : titre + meta + H1 + lead + suppression du doc collé. **Non touché** (hors scope T2-URGENT, batchs dédiés ultérieurs) : tableau de prix inventé (lignes 392-399 vs PRICING.md 65 €/h + Z1-Z6), claims R12 « 24h » (chantier X-R12), FAQ/H3 long-tail.
- **Conformité** : R4 zéro invention (4 modifs verbatim, 0 nouvelle affirmation ajoutée), R5 (0 streetAddress, NAP inchangé `+351****4451`), R7 (PR draft, 0 merge, attente GO Philippe), R8 (témoins avant/après : `<!DOCTYPE>` 2 → 1, `</html>` 2 → 1, bytes 56787 → 22785), R11 (collectif « a nossa equipa » préservé), R12 (0 slogan 24h/7d ajouté), R13 (0 mention DGEG/TRIESP/wallbox CNR). R-WT (worktree `~/work/Sites/canalizador-norte-reparos/.worktrees/t_0ce9e9ed`, branche locale).
- **Témoins R8 (avant/après — verbatim, 1 motif par commande)** : query verbatim « como desentupir o lava loiça » dans title + meta description + H1 + lead `<p>` : 0 → **4**. `<!DOCTYPE html>` dans le fichier : 2 → **1**. `</html>` dans le fichier : 2 → **1**. Bytes : 56787 → **22785** (-60 %, doc collé retiré). Marqueurs de conflit git : `grep -nE '^(<<<<<<<|=======|>>>>>>>)' client/public/blog/como-desentupir-lava-loica.html` = 0. DGEG/TRIESP/wallbox côté CNR : 0. Le second doc retiré ne contenait aucune info SEO nouvelle (mêmes FAQ + H3 « Como desentupir lava loiça? », déjà couvert par doc 1).
- **Vérifications** : `git diff --check` propre, commit `4130d6113d` atomique (1 commit, 1 fichier), push OK sur `github/fix/cnr-rankpush-como-desentupir-lava-loica-t_0ce9e9ed`, PR **#386 draft ouverte** (https://github.com/taffrand-gif/canalizador-norte-reparos/pull/386), base=main, +4/-104.
- **Statut** : ⏸ PR draft #386, 0 merge (R7), attente GO Philippe. Branche `fix/cnr-rankpush-como-desentupir-lava-loica-t_0ce9e9ed` basée sur `github/main` 264bef01cc, push OK. Mesure à J+7 via `gsc-trajectoire-cron.sh` : **win si `como desentupir o lava loiça` passe pos < 4 ET/OU clics > 0** (la requête était déjà à pos 14.3 = fenêtre top3 théorique, le fix vise à passer le snippet en CTR > 0). Si pos > 14.3 et clics 0 à J+14 : **rollback doctrinal possible** (revert PR #386, le doc 2 sera restauré mais le title sera ré-aligné manuellement). Tâche `t_0ce9e9ed` à clore après push + ouverture PR draft.
